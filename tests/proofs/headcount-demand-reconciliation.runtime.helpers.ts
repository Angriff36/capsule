/**
 * Shared harness for the AC-390 demand-reconciliation runtime proof: roles,
 * the planned-event seed, and one published component-dish pair per call so
 * each dish line drives exactly one live IngredientDemand. Demand readers are
...
export async function createPlannedEvent(
  ...
  const client = await run(api.mutations.Client_createViaRegister, {
    clientType: "company",
    companyName: `Demand survival client ${tenantId} ${title}`,
  });
  const event = await run(api.mutations.Event_createViaPlanEngagement, {
    ...
    primaryContactName: "Casey Demandsheet",
    budgetAmount: 3000,
    quotedPrice: 4500,
  });
...
export async function seedComponentDishPair(
  ...
  eventId: string,
  nameSalt: string,
): Promise<{ lineId: string; ingredientId: string }> {
  ...
  const ingredient = await runKitchen(
    ...
    {
      name: `AC-390 demand ${nameSalt} ingredient ${tenantId}`,
      unit: "each",
      costPerUnit: 1,
      allergens: [],
      category: "dry",
  ...
  const component = await runKitchen(api.mutations.Component_createViaDraft, {
    name: `AC-390 demand ${nameSalt} base ${tenantId}`,
    yieldQuantity: 1,
    yieldUnit: "portion",
    batchMultiplier: 1,
  });
  ...
  const dish = await runKitchen(api.mutations.Dish_createViaIntroduce, {
    name: `AC-390 demand ${nameSalt} dish ${tenantId}`,
    portionSize: 1,
    portionUnit: "portion",
    category: "bread",
  });
  ...
  return { lineId: line.docId, ingredientId: ingredient.docId };
}
