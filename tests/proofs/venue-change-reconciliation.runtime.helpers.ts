/**
 * Shared harness for the AC-390 venue-reconciliation runtime proof: roles,
 * the planned-event seed, one seeded current packet revision, and readers for
 * the live revisions and the Event's venue snapshot. Assertion-free; the
 * test file owns every expect().
 */
...
export async function createPlannedEvent(
  ...
  const client = await run(api.mutations.Client_createViaRegister, {
    clientType: "company",
    companyName: `Venue survival client ${tenantId} ${title}`,
  });
...
export type EventVenueSnapshot = {
  venueId: string | null;
...
/** The Event's venue snapshot (venue change slice), or a thrown error. */
export async function readEventVenue(
  actor: Role,
  eventId: string,
): Promise<EventVenueSnapshot> {
