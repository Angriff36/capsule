/**
 * Shared harness for the AC-390 style-change reconciliation runtime proof:
 * roles, the planned-event seed, one seeded current packet revision, a
 * registered service style, and readers for the live revisions and the
 * Event's style snapshot. Assertion-free; the test file owns every expect().
 */
...
export async function createPlannedEvent(
  ...
  const client = await run(api.mutations.Client_createViaRegister, {
    clientType: "company",
    companyName: `Style survival client ${tenantId} ${title}`,
  });
...
/** A catalog service style for the tenant (the events role may register
 * styles). Returns the style id. */
export async function registerServiceStyle(
  proof: Proof,
  tenantId: string,
  name: string,
  code: string,
): Promise<{ styleId: string }> {
  const run = runner(proof, rolesFor(proof, tenantId).events);
  const style = await run(api.mutations.ServiceStyle_createViaRegister, {
    name,
    code,
    sortOrder: 10,
  });
  return { styleId: style.docId };
}

export type EventStyleSnapshot = {
  serviceStyleId: string | null;
  serviceStyleName: string | null;
  version: number;
};

/** The Event's service-style snapshot (style change slice), or a thrown
 * error. */
export async function readEventStyle(
  actor: Role,
  eventId: string,
): Promise<EventStyleSnapshot> {
  ...
  return {
    ...
    serviceStyleName: event.serviceStyleName ?? null,
