/**
 * Shared harness for the AC-388 purchasing buyer-quantity override survival
 * runtime proof: roles, the weekly-purchasing seed (vendor, config, two
 * ingredients, published components, dishes, approved event, weekly draft
 * lines), and draft-line readers. Assertion-free; the test file owns every
 * expect().
 */
...
export const S = {
  // Same week as the weekly-purchasing proof so the week key is stable
...
export async function seedWeeklyOrder(
  ...
  const vendor = await runProcurement(api.mutations.Vendor_createViaOnboard, {
    name: `Weekly buyer survival vendor ${tenantId}`,
    paymentTermsDays: 14,
  });
  ...
  const saltLike = await runKitchen(
    ...
    {
      name: `AC-388 salt-like ${tenantId}`,
      unit: "each",
      costPerUnit: 1,
      allergens: [],
      category: "dry",
  ...
  const pepperLike = await runKitchen(
    api.mutations.Ingredient_createViaIntroduce,
    {
      name: `AC-388 pepper-like ${tenantId}`,
      unit: "each",
      costPerUnit: 1,
      allergens: [],
      category: "dry",
  ...
  const client = await runSales(api.mutations.Client_createViaRegister, {
    clientType: "company",
    companyName: `AC-388 buyer survival client ${tenantId}`,
  });
  ...
  const lines = await draftLines(roles.procurement, tenantId, draft._id);
...
/** Live, non-cancelled lines of one draft, sorted by ingredient id so A/B
