...
describe("plain words on leftover culinary import constraint copy", () => {
  it("keeps leftover culinary import matched-ingredient copy free of confirmed-new / ingredient-id jargon", () => {
    ...
    // The old confirmed-new / ingredient-id jargon refusal is gone.
    expect(visible).not.toContain(
      "Only exact, confirmed existing, or attached confirmed-new lines may carry a matched ingredient id",
    );
    ...
    // Already-landed copy from this same import file stays.
    expect(visible).toContain(
