...
const SOUR_CREAM_DISH = "/kitchen/dishes/ks78f47fy9f0k8xhcnkwb3kd5n8crx7c";
...
describe("218 leftover: Ctrl+A is not a nav/collapse chord", () => {
  it("treats Ctrl/Cmd+A as select-all in the field, not collapse", () => {
    ...
    expect(editor).toContain("applyRecipeSearchSelectAll");
    expect(shell).toContain("isSelectAllChord");
    expect(tab).toContain("dishPath");
    expect(tab).toMatch(/<Link[\s\S]{0,120}dishPath/);
    ...
    expect(appRouteForKeydown(ctrlA)).not.toBe(SOUR_CREAM_DISH);
    expect(appRouteForKeydown(cmdA)).not.toBe(SOUR_CREAM_DISH);
  });

  it("Ctrl+A in Search catalog selects the field and does not hop the dish link", () => {
    ...
    applyRecipeSearchSelectAll(event);
