...
describe("plain words on leftover order-line receipt-price copy", () => {
  it("keeps the negative-price refusal free of jargon", () => {
    ...
    expect(visible).not.toContain("Receipt unit price cannot be negative");
    ...
    expect(summary).not.toContain("Receipt unit price cannot be negative");
    // The command-level half of this refusal lands in generated mutations.
    const mutations = readFileSync("convex/mutations.ts", "utf8");
    expect(mutations).toContain(NEW);
    expect(mutations).not.toContain("Receipt unit price cannot be negative");
    // Already-landed copy on this same file stays.
    expect(visible).toContain(
      "This order line's received amount has to be more than zero. Enter how much arrived.",
    );
    expect(visible).toContain(
