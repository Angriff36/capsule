...
describe("plain words on leftover equipment recount copy", () => {
  it("keeps the negative equipment recount refusal free of jargon", () => {
    const manifest = readFileSync("src/facilities/equipment.manifest", "utf8");
    // Strip // comments the same way the culinary leftover tests do.
    const visible = manifest.replace(/(^|[^:])\/\/[^\n]*/g, "$1 ");
    const NEW =
      "This equipment's counted amount can't be negative. Use zero or more.";
    // The old jargon refusal is gone from the authored equipment file.
    ...
    expect(mutations).not.toContain("Recount quantity cannot be negative");
