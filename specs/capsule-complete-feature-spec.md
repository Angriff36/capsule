...
| 4.1 | Event creation fields | 🟡 | Date/guests/venue + occasion, service-style, salesperson, referral selectors live (e871648). Archive/reactivate and duplicate exist (2026-09-22; AC-239). Remaining: salesperson/referral persistence proof (AC-234) and owner Confirmed/final mapping. |
...
| 10.4 | Waste tracking | ✅ | `demand.manifest` WasteRecord (reason enum, costImpact, on-hand decrement). |
| 10.5 | Inventory | ✅ | `stock.manifest` (on-hand/par/reorder, movements, reservations w/ event release). |
| 11.1 | Equipment inventory | 🟡 | Ownership + condition + value present; **no location field**. |
| 11.2 | Pack list templates | 🟡 | Per-event PackList (auto-open on approval); **no template / service-style linkage**. |
| 11.3 | Availability & movement | ✅ | EquipmentReservation (reserved→checked_out→returned) + availability calc. |
| 11.4 | Maintenance | ✅ | EquipmentMaintenanceTask + immutable EquipmentServiceEntry. |
| 12.2 | QuickBooks | ✅ | `qboSync.ts` (OAuth, push invoices+payments). |
| 12.3 | Nowsta | ❌ | No Nowsta code; payroll is CSV export (Gusto/ADP/Paychex). |
| 12.4 | Google Calendar | ✅ | `googleCalendar.ts` (OAuth, push approved events). |
| 12.5 | Email | 🟡 | Manual `client-communication` log + outbound transactional email. No connected inbox/threading. |
