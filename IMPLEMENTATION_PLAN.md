...
**Cross-model review (codex gpt-5.6-sol; author = Claude/Opus):** round 1 returned 2× P2 (no P1; reviewer noted "no new guard or policy tedium" — the catering-not-a-bank gate passes). Both fixed this revision:
- **P2 — legacy boolean stamping (`VenueDetailPage.tsx`).** The edit-form checkboxes (`defaultChecked={venue.X ?? false}`) meant a venue created before a field existed (stored `undefined`) rendered unchecked, and ANY edit serialized the unchecked box as `false` — the display would then assert "✗ Power/Water/Stairs" (and the same for the pre-existing Parking/Freight/Storage) for a value nobody ever set, potentially misdirecting packing/staffing decisions. Fix: a `logisticsBoolean(checked, stored)` helper preserves `undefined` (unknown) for unset values and only records `false` when a value was already defined; applied to all 6 logistics booleans (3 new + 3 existing) for consistency. Create form unchanged (fresh records carry the user's unchecked=no choice; the legacy-unknown concern only applies to edits of pre-existing records).
- **P2 — unauthorized deferral wording (`IMPLEMENTATION_PLAN.md`).** This entry originally called the remaining §8.1 sub-entities "deliberately deferred," which violates `docs/architecture/no-invented-deferrals.md` ("Not built ≠ owner-deferred"). Reworded to "not built yet (still open)."

**Honest scope notes:** (1) Blank string fields can't be CLEARED by blanking in the edit form (blank → `"" || undefined` → `undefined` → `patch` drops → preserves the stored value) — identical pre-existing behavior for every Venue string field, not introduced here; a clear-on-blank change would alter existing-field behavior repo-wide and is out of scope. (2) `restrictions` is a single textarea, not a chip-list — the spec names no structure for it; simplest faithful form. (3) No focused test added for the new-field round-trip / legacy-undefined case — AGENTS.md forbids adding tests unless the owner asks; covered by the passing `bun run check` gate and verified-by-consistency with the existing logistics fields.
