...
| **Slice 2** | 🟡 Partial | 1 | ~40% | ExternalRecordLink ✅, ImportRun ✅, Reconciliation UI ✅, Dashboard ✅, Cutover ✅, **venues + contacts + events commit/revert ✅ real (2026-07-26)** | commit/revert were silent-false-success stubs — now REAL for venues/contacts/events (events resolve external client/venue ids from prior imports; events land at stage `planning` with raw TPP status preserved); leads/menus/payments materialization NOT wired; no TPP bulk export to validate against. (Prior "100% Complete" was FALSE — see Changes This Update.) |
...
|----------|------|--------|--------|--------------|-----------|--------|
| ~~1~~ | **Import Framework** | Large | Critical | None | Foundation for entire TPP migration - blocks Slice 2 | 🟡 PARTIAL (2026-07-26): entities + reconcile UI + dashboard + cutover + **venues + contacts + events + leads commit/revert REAL** (`convex/importCommit.ts`); menus/payments still NOT wired (cross-dataset ID resolution + menus parser). Prior "✅ DONE" overstated. |
| 2 | **Import Datasets** | Medium | Critical | None | Events/Contacts/Leads/Menu/Venues/Payments import - 2,103 TPP events | 🟡 PARTIAL (2026-07-26): parsers exist for events/contacts/leads/venues/payments (`convex/tppParser.ts`); **VENUES + CONTACTS + EVENTS + LEADS** materialize end-to-end via `importCommit.ts` (contacts → person-type `Client`; events → `Event` with cross-dataset client/venue resolution; leads → company-type `Lead` pre-client inquiry). Menus/payments not wired. Prior "✅ DONE 6 datasets" was FALSE. |
