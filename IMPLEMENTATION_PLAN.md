...
- `convex/tppParser.ts` — added `TppMenuRecord`, `ParsedCapsuleMenu`, a singular `parseTppMenu` (slugifies Name → externalId; `parsePortionSize` extracts a leading int from free text like "75 servings", default 1; `portionUnit "portion"`; `mapTppAllergens` normalizes display names Eggs/Shellfish/Dairy/Wheat-Gluten/Tree Nuts → the closed AllergenCode enum, dropping unknowns), a batch `parseTppMenus` (validates non-empty Name, which doubles as the external id), and the helpers.
