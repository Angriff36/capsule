...
**Why it matters:** these drive real ops decisions per spec line 372 — power/water/stairs/load-in drive PACKING (generator? water haul? stairs vs elevator?), waste rules drive ops, permits/insurance drive compliance, restrictions drive proposal warnings. Ops previously had to cram all of it into one textarea and re-read prose every time; now each is a structured, at-a-glance field. Completes the §8.1 venue-profile field list (remaining §8.1 items are the high-effort sub-entities — room/space, attachments/photos, scorecard metrics — deliberately deferred).
...
**Honest scope notes:** (1) Blank string fields can't be CLEARED by blanking in the edit form (blank → `"" || undefined` → `undefined` → `patch` drops → preserves the stored value) — this is the identical pre-existing behavior for EVERY Venue string field (kitchenAccess, logisticsNotes, …), not introduced here; a clear-on-blank change would alter existing-field behavior repo-wide and is out of scope. (2) Power/water/stairs are booleans (structured yes/no for the packing decision); free-text nuance goes in `logisticsNotes` (spec line 372: "use notes for genuinely unstructured context"). (3) `restrictions` is a single textarea, not a chip-list — the spec names no structure for it; simplest faithful form.
