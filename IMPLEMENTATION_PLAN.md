...
- **Finding 2 (audit error leaked line description + prices pre-auth) — FIXED.** The override-audit threw `"<description>" is priced at X but …` before `Proposal_send`'s salesAccess guard ran, so a caller holding a foreign proposal id could read its lines/prices. Now the seam fetches the proposal first (fail-fast, non-disclosing "Proposal not found"), skips foreign-tenant / missing / unpriced menuDishes, and throws a non-disclosing message ("One or more catalog-linked lines have an unapproved price override …"). The inline UI still flags each divergent line, so operator UX is preserved.
- **Finding 3 (unguarded `menuDishId` → foreign dish leaks into snapshot / bypasses audit) — FIXED.** A foreign-tenant or unpriced linked dish was read by id and its `sellingPrice` snapshotted as `catalogPrice` / compared in the audit. Now BOTH the audit scan AND `buildProposalRevisionSnapshot` require `menuDish.tenantId === proposal.tenantId` (and a non-null `sellingPrice`) before treating a link as a priced catalog entry; foreign / missing / unpriced links carry no `catalogPrice` and are never audited. (`menuDishId` stays a uuid? column — manifest-level same-tenant FK enforcement is a generator feature, not in-repo; the authored seam validates at use, the only place a cross-entity tenant check is expressible.)
- **Finding 1 (command-API `Proposal.send` bypasses the audited seam) — SYSTEMIC / generator-level, NOT fixable in-repo.** `convex/http.ts:1324` dispatches `"Proposal.send"` → the generated `Proposal_send` (do-not-edit), and the agent resolver (`src/agent/CapsuleCapabilityMutationResolver`) maps the capability to the generated mutation name with no per-command override. A manifest guard cannot express the audit (it would have to fan-in to read `MenuDish.sellingPrice` from a `ProposalLineItem` guard). So the cross-entity override-audit — like capture-on-send before it — is ONLY expressible in authored seams, which the command API structurally bypasses by targeting generated mutations directly. Same class as #111 / the idempotency-framework findings. The UI operator send path (the primary surface) is fully audited + captures; the bypassed path is agent/command-API only (JWT-gated, internal, and `Proposal.send` is not even in the agent AC set). **Escalated** (not auto-overridden; `REVIEW_GATE=0` is human-only per the merge gate).
