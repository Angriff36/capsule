...
Documented follow-ups (scale/edge, not blocking a single-org catering deployment): full per-caller rate limit (needs a counter table+TTL), host-based multi-tenant resolution (deployment is single-org today), compound `tenantId`/`dedupKey` index for O(1) dedup (currently filters after a tenant-scoped collect), emit `QuoteSubmitted` manifest event in the seam, validate `serviceStyleId`/`occasionId` against the resolved tenant, match only active (not archived) clients by email, collision-resistant per-submission idempotency key, persist partial IDs before marking a conversion failed (no manifest partial-set command exists today), and sales-only queue gating — deliberately NOT added here because the queue exposes the same client PII (`ClientsPage`) already exposes app-wide to all authenticated staff; a one-off gate would be inconsistent over-gating per `docs/architecture/domain-gating-restraint.md` (CRM read-scoping is an app-wide concern, not this diff's).
