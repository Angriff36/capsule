...
- [x] RR-1 — Honest measurements (AC-042): implemented 2026-09-06 on `ralph/recipe-review`. Strict `UnitOfMeasureMapper.resolve` (unknown/blank/size-word units → null; legacy `map` kept permissive for prep-sheet compat); parsed/review quantity, unit, yieldQuantity, yieldUnit are `number | null` / `UnitOfMeasure | null` with `unitRaw`/`raw` preserved; `reviewMeasurementIssues` + `isMissingQuantity` in ComponentImportTypes; `reviewIsReady` combines matching + measurement issues; finalizer validates then narrows to the required-number projection before any write; CSV parser keeps yield gaps null instead of defaulting 1 portion; panes show a correction block with jump-to-first-issue and disable Save until measurements are complete; agent enter bridge warns on measurement gaps and refuses dish introduce without a measured yield. Tests: tests/component-text-parser.test.ts + new tests/component-import-review-readiness.test.ts (25 green). Full `bun run test` 162 files / 1390 tests, typecheck, format:check, `bunx vite build` all green. Preview verified on http://127.0.0.1:7813 (7812 is held by the sibling capsule-ralph checkout; .ralph.env updated).
- [x] RR-2 — Executable durable lifecycle (AC-043): implemented 2026-09-06 on `ralph/recipe-review`. `src/culinary/component-import.manifest` repairs: suggestExactMatch stores the candidate without the old-FK guard; confirmExisting validates the supplied id against the stored suggestion through the tenant-scoped belongsTo (active + not deleted) plus a param-equals-stored guard; attachCreatedIngredient stores the created link; complete() validates the resulting component (tenant-scoped hydration) and every stored line link (hasMany + per-line belongsTo + tenant compare) before any completion write. New `reviewRevision` (monotonic, default 0) on ComponentImport; `reviseReview` (header, bumps revision, reviewing-only, never touches rawSourceText/source fields); `reviseMeasurements` on lines (requires the import's current revision via self.import traversal, stamps revisedAt); `beginFinalization(expectedReviewRevision)`; `resumeReview` also demotes ready→reviewing (clears readyAt) so post-approval corrections must be re-approved; lineConfirmedNewWithoutMatch/lineMatchedIngredientAllowed constraints updated for the attached-confirmed-new end state. Regenerated via `bun run manifest:regen` + codegen. Proof: tests/proofs/component-import-review.runtime.test.ts (4 tests, full lifecycle + stale-revision/foreign-tenant/foreign-component denials without writes + recovery + immutability). Gates: bun run test 163 files/1396 tests, typecheck, format:check, bunx vite build all green. Compiler note: update commands cannot hydrate a supplied FK param through a relation in the pinned compiler — validation rides the draft-visible init inputs (stage) or the stored-value hydration at the next gate (suggest→confirm, complete()); no generated file was hand-edited.
- [x] RR-3 — Atomic persistence and recovery (AC-044): implemented 2026-09-06 on `ralph/recipe-review`. `convex/lib/culinaryOperations.ts` gains `createComponentImportReview` (upload+recordParse+beginReview+stage all lines in one transaction, returns importId/reviewRevision/lineIds), `saveComponentImportReview` (header reviseReview + changed-line reviseMeasurements in one transaction, stale expectedRevision conflicts with no writes, omitted fields resolved to stored values because generated revise commands wipe omitted optionals — found the hard way, preserved null-as-unknown), and a review-aware `importComponent` path (`review: {importId, expectedRevision}`): validates status/revision/yield/line measurement presence and projection-vs-stored drift (quantity, unit, ingredient decision, created name) before any write, introduces + attaches created ingredients, records the component, completes the same import, and stores the exact request inside the materialization receipt (family `componentImportReview`) so identical replay after a lost acknowledgement returns the same result while the same operation key with a changed request conflicts. Legacy no-review path unchanged (pack/clone/restore tolerant replay semantics untouched). `src/features/kitchen/import/ComponentImportRepository.ts` (new) exposes create/load/save through governed transactions only, with pure request builders (buildCreateReviewRequest/buildSaveReviewRequest/mapStoredReview) reused by the proofs; ComponentImportTypes gains `reviewRevision`; the finalizer passes `{importId, expectedRevision}` through its atomic port. Manifest: ComponentImport + `csvSheetText`/`csvLinesText` (upload params too) so CSV pairs stay separately recoverable; regenerated via `bun run manifest:regen`. Proofs: safe-culinary-operations.runtime.test.ts +1 durable test (save/load identity, conflict, finalize/replay/changed-payload, foreign tenant, injected rollback + recovery), component-import-finalize.runtime.test.ts +1 repository test (legacy tests kept). Gates: bun run test 163 files/1398 tests, typecheck, format:check, bunx vite build, manifest-regen-check all green. Note: SourceFingerprint.digest stays display/dedup only — identity is importId+revision+operation key.
- [x] RR-4 — Resumable normal UI (AC-045): implemented 2026-09-06 on `ralph/recipe-review`. `/kitchen/components/import` gains `?importId=<id>` resume with an id-plausibility guard (malformed ids render the unavailable state instead of crashing `v.id()` validation), a compact resumable-import list (tenant `useListComponentImport`, non-completed/cancelled, newest first), truthful Saving/Saved/Save-failed/conflict states, and a read-only `ComponentImportSourcePanel.tsx` (raw paste/sheet text, separately stored CSV pair, filename, import identity, status label; text only, never HTML) shown on the workbench and on Component detail for completed imports (absence on native components renders nothing). Culinary integration guard honored: no direct `convex/react` usage in kitchen features — the import's lines are `useListComponentImportLine()` filtered client-side. Persistence went through the RR-3 seams plus three additive extensions: `createComponentImportReview` stores the workbench's initial exact/possible/new match confidence; `saveComponentImportReview` persists changed match decisions (suggest/confirm/mark/reset through generated line commands inside the one transaction), name corrections (`ComponentImportLine.reviseMeasurements` gained optional `parsedIngredientName`; omitted fields still resolve to stored values because generated revise commands wipe omitted optionals — direct proof callers now restate the name), removed lines (`ComponentImportLine_discard` with reason), and recomputes `resolvedLineCount` from storage so a discard counts as a final outcome and approveReview stays honest; auto-resume covers failed as well as ready reviews. `ComponentImportRepository` builds/diffs decisions, names and discards (`buildSaveReviewRequest`), carries raw source through `mapStoredReview`, and gains `adopt()` for the reactive workbench load. Finalize on a durable review pre-saves unsaved edits first; a failed save aborts finalize honestly. Tests: tests/features/kitchen/component-import-review.test.ts (9 mounted jsdom tests incl. conflict retention + reload, completed link, missing/denied, source-as-text, detail provenance) and tests/proofs/safe-culinary-operations.runtime.test.ts +1 decisions/name/discard proof. Gates: bun run test 164 files/1408 tests, typecheck, format:check, `bunx vite build`, manifest-regen-check all green. Preview verified http://127.0.0.1:7813 serves this checkout (7812 = sibling capsule-ralph, do not kill). Browser-level qualification (desktop/360px, keyboard, live backend) stays in RR-5/AC-046.
- [x] RR-5 — Qualification and handoff (AC-046): implemented 2026-09-06 on `ralph/recipe-review`. Browser qualification ran against THIS checkout's own anonymous local Convex backend (127.0.0.1:3212, isolated from the shared 3210 owned by the primary checkout) and the 7813 preview, signed in through the product's own staff sign-in ticket (`convex/lib/clerkSignInTicket.ts`; password sign-in on the dev instance demands an emailed new-device code). Committed runnable procedure `scripts/recipe-review-browser-qualification.ts` (21 checks; scratch playwright-core install, run under Node — Bun cannot complete playwright's CDP pipe launch here; RR5_EXECUTABLE pointed at the cached ms-playwright chromium because system Chrome hung under CDP). Final pass 21/21 PASS (2026-09-07T00:55:39Z, exit 0): denied-when-signed-out, authenticated paste + CSV-pair correction with honest unknown-unit/missing-yield issues blocking finalize, durable save with ?importId, reload persistence, read-only original source on workbench + component detail, missing price staying incomplete cost ("1 / 3 lines" + named gaps), 360px mobile panes, two-tab stale-revision conflict keeping local edits + reload-adopted stored version, interrupted-acknowledgement finalize reopening completed, signed-out denial with no source leakage. THE QUALIFICATION CAUGHT A REAL DEAD-END: nothing promoted a completed review to `ready`, so the ordinary paste→correct→save→finalize flow failed server-side with "component import is reviewing; only ready or finalizing reviews can be finalized" — the imperative proofs called approveReview explicitly and the mounted jsdom tests mock the hooks, so only a real browser could find it. Fixed: `createComponentImportReview` now recomputes `resolvedLineCount` after staging decisions (mirroring the save transaction's ledger recompute), and `ComponentImportRepository` gained an optional `approveReview` port fired after a completed create/save, wired in `ComponentImportPage` to the generated `useComponentImportApproveReview` hook (kitchen integration guard respected; later corrections still demote via resumeReview and re-promote on the next completed save). Evidence: `codex-plans/production-readiness-next/recipe-review-verification.md` (procedure + dated results + honest limits); raw log/results/screenshots under gitignored `.artifacts/rr5/`. Gates after the fix: bun run test 164 files/1408 tests, typecheck, format:check, `bunx vite build`, lint_specs (19), manifest-regen-check all green. Local dev environment only; no production/deployment/full-PR14 claim; historical AC-006/013 J receipts still missing (ANTHROPIC_API_KEY unset). AC-046 → PASS; with AC-042..045 already PASS the delivery's criteria are complete pending owner review/release.
