...
**Fix (smallest spec-faithful diff; 1 authored Convex projection + the shared PDF type):** `convex/clientPortal.ts` `getEvent` now fetches each accepted proposal's `proposalLineItems` via the existing `by_proposalId` index (bounded — one focused query per accepted proposal for this event, NOT a tenant-wide scan) and attaches a `pricingLines` array. The projection carries ONLY client-safe fields (`description`, `pricingBasis`, `unitPrice`, `quantity`, `unit`, `amount`) — internal cost/margin + the override-audit fields (`menuDishId`/`overrideReason`/`catalogPrice`) stay private, matching what the operator PDF exposes. It flows through with **zero** `ClientPortalPage.tsx` changes: the portal download handler already spreads `...proposal` into `downloadProposalPdf`, the portal proposal type is already `ProposalPdfRecord` (which already declares `pricingLines?: PricingLinePdf[]`), and `generateProposalPdf` renders `proposal.pricingLines ?? []` — so the client's downloaded accepted-proposal PDF now itemizes every priced line. `src/features/clients/proposalPdf.ts` adds an optional `amount?: number | null` to `PricingLinePdf`; the builder prefers it when present (frozen stored amount — §5.5 reproducibility / §5.4 "stored amount is authoritative"), falling back to the central calc when absent (unchanged operator/draft behavior).
...
**Verification:** `bun run check` GREEN (exit 0) — toolchain, ownership ledger, typecheck 0 (`PricingBasis` schema literal union == `src/lib/pricing.ts` type, so `line.pricingBasis` needs no cast), format clean, secrets, test:coverage, build ok, baseline-decay ok. Runtime path verified by inspection: `getEvent` → `proposalLineItems` (by_proposalId, per accepted proposal) → `acceptedProposals[].pricingLines` → `ClientPortalPage` `...proposal` spread → `downloadProposalPdf` → `generateProposalPdf` `pricingLines ?? []` (prefers stored `amount`, else central calc). No tests added (authored seam projection; AGENTS.md: do not add tests unless the owner asks). The repo's recurring GREEN-but-broken failure modes do NOT apply (pure read+project of an existing entity through an existing render path; no creation command, no guard, no relation key, no datetime param).

**Cross-model review (codex gpt-5.6-sol, author=Claude/Opus) — ran on `origin/main..HEAD`; 2 findings, BOTH FIXED this revision:**
- **P2 (unbounded tenant-wide read) — FIXED.** The first cut loaded ALL of a tenant's `proposalLineItems` via `by_tenantId` + `.collect()` on every anonymous portal load; for a tenant with sales history that grows past Convex read limits and would break every portal link for that tenant. Now lines are fetched per accepted proposal via the existing `by_proposalId` index (bounded — accepted proposals per event ≈ 1).
- **P1 (render from frozen revision, not live-recomputed) — FIXED (carry stored amount).** The first cut let the PDF re-derive each line amount via the current `computeProposalPricing`; a future calc change could make the breakdown not foot to the accepted total. Verified first-hand that accepted proposals are IMMUTABLE (all three line commands `addLine`/`reviseLine`/`removeLine` guard `self.proposal.status == "draft"`), so live lines == frozen accepted lines — no current divergence. To close the future-risk and honor §5.5 reproducibility + §5.4's "stored amount is authoritative" (single source of truth), the projection now carries the stored `amount` and `generateProposalPdf` prefers it when present; the operator/draft path is unchanged (amount optional → falls back to the calc).
