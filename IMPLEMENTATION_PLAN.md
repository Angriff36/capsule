...
Documented follow-ups (scale/edge, not blocking a single-org catering deployment): full per-caller rate limit (needs a counter table+TTL), host-based multi-tenant resolution (deployment is single-org today), compound `tenantId`/`dedupKey` index for O(1) dedup (currently filters after a tenant-scoped collect), emit `QuoteSubmitted` manifest event in the seam, validate `serviceStyleId`/`occasionId` against the resolved tenant, match only active (not archived) clients by email, collision-resistant per-submission idempotency key.
