...
**⚠ Push status:** will be gated by the pre-push `review-gate-hook` over `origin/main..HEAD` (6 unpushed commits). The cumulative diff still carries the pre-existing systemic findings (#111 cross-tenant `*ByTenantId`, generator-level idempotency) that are NOT introduced by this increment and not fixable in-repo; this focused increment adds no new guard/policy/approval and reuses proven patterns. If the gate denies, escalate to the human (`REVIEW_GATE=0` is human-only per the merge gate) — do NOT override autonomously.
