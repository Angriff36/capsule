...
**NEW BLOCKER 1 — [#113](https://github.com/Angriff36/capsule/issues/113): `bun run seed` is a no-op.** The generated `scripts/seed-convex.ts` only *exports* `seedConvex(deploymentUrl)` (line 8) with no top-level call/`import.meta.main`/argv handling, so `bun scripts/seed-convex.ts` defines the function and exits 0 with no output. And even when invoked it emits 124 `// skip <Entity>: no create command in IR` lines against only 12 real mutation calls — `Client`, `Event`, `Venue`, `Dish`, `Person` are all skipped, i.e. the whole event spine. The seed binding appears to look for a literal `create` command rather than the promoted create verb (`Event_createViaPlanEngagement` etc. plainly exist). **Consequence: the reference/config tables are empty** — `organizations`, `serviceStyles` and `occasions` have zero rows (`venues`/`clients`/`events`/`people` DO have data; an earlier note in this file claimed "completely empty", which was wrong — it came from a broken row-count parser). An empty `organizations` table is the root cause of both the empty Service Style/Occasion dropdowns AND the "Unable to process quote" failure on `/quote`: `convex/quoteBuilder.ts:107-115` resolves the public tenant as the first **active organization** and throws when there is none. Generator-level; not hand-fixable here.
...
**Why it matters:** closes the §4.4 "failed parsing appears in a retryable sync-error queue" clause and the integrations "error visibility" done-when. A malformed provider delivery no longer disappears — it lands in a retryable queue a staff member can see, re-run, or dismiss, with full source provenance (the verbatim failed input). The queue is provider-neutral and cross-cutting: the message ingest records here today, and other provider syncs (QBO/Calendar/SMS) may record here the same way.

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok`: toolchain, builder ownership, proof:emit, all 10 manifest-slice integration guards, manifest-registry-pin, typecheck 0, format clean, secrets, test:coverage (65 files), build ok (`MessageInboxPage` + `SyncErrorsPanel` chunks built), baseline-decay ok. Runtime write-path verified by inspection (the recurring GREEN-but-broken trap does NOT apply): the create uses the generated `SyncError_createViaRecord` governed-creation hook (allocates a draft, returns `{docId}` — the proven `ClientCommunication_createViaRecord` shape); the `recordedAt == null` creation guard is satisfiable on a fresh draft and is the idiom that makes `record` allocate at all (see generator learning below); `markResolved` passes `{docId, version}` (OCC token); auth propagates through `ctx.runMutation` (same identity, like `importCommit`); no `context.timestamp`/`self.id`-in-mutate/`_createVia*`-docId trap is touched. No tests added (authored manifest + additive seam/UI cloning shipped siblings; AGENTS.md: do not add tests unless the owner asks).
...
**2026-07-26 — §9.3 Hiring Pipeline — NEWLY IMPLEMENTED (closes the last Slice-4 HR gap; the detailed ❌ row was truthful, now built):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** One new authored manifest file (two entities) + regen + a KM-JSON parser + an idempotent authored ingest seam + a client wrapper + a page + route + nav + two test-mapping lines. No money mutation, no new guard/policy beyond the existing `workforceManageAccess`.

**Finding (verified first-hand against code this turn):** spec §9.3 ("Map the KM interview tool JSON into Capsule's existing candidate/interview model. Preserve source IDs and raw response references. The pipeline supports the current business stages—application, screening, interview, decision/offer, hired or rejected—using repo terminology. Re-importing the same candidate/interview updates the source-linked records without duplication.") was entirely unbuilt — confirmed by exhaustive search: no `candidate.manifest`/`interview.manifest`, no `candidates`/`interviews` table, no page/route. The §9.2 RoleScorecard + §9.5 OneOnOne parent/child patterns (shipped the prior two increments) supplied the exact manifest+page+route blueprint to clone.

**Fix (smallest spec-faithful diff — clones the §9.5 OneOnOne/OneOnOneAction parent/child pattern + the importCommit idempotent-create shape):**
- `src/workforce/hiring.manifest` — `Candidate` (the applicant; `apply` create command; pipeline `stage` enum application/screening/interview/decision/hired/rejected; `advance`/`reject`/`hire` commands) + `Interview` (closable child of Candidate; `schedule` create + `recordOutcome`; outcome pending/passed/failed). Source fidelity per §9.3: each entity carries `sourceSystem` + `external*Id` + `rawSourceData` (the raw KM response as JSON). Why two entities, not stage-on-candidate: interviews are scheduled/conducted over time with their own outcome — a child row lets an interview outcome be recorded without rewriting the candidate (same parent/child split as OneOnOne/OneOnOneAction). No `transition` state-machine is declared (per `docs/architecture/domain-gating-restraint.md`): the enum vocab + command guards are enough; no operator is policy-denied moving a candidate forward or correcting a stage.
- Registered in `src/app.manifest`; `bun run manifest:regen` produced `candidates` + `interviews` tables (with `by_candidateId` index) + 6 command hooks (`Candidate_createViaApply`/`advance`/`reject`/`hire`, `Interview_createViaSchedule`/`recordOutcome`) + 9 React hooks.
- `convex/kmParser.ts` (new) — `parseKmCandidates`: accepts a bare array / `{Candidates:[]}`/`{candidates:[]}`, liberal on key casing (Pascal/camel/snake, mirroring `parseTppPackList`); `mapKmRole`/`mapKmStage`/`mapKmOutcome` map freeform KM text to CapsuleRole/CandidateStage/InterviewOutcome literals (safe defaults so a re-import never crashes); the verbatim KM record is preserved on `raw` (→ `rawSourceData`) so §9.3's "preserve source IDs and raw response references" holds for fields Capsule does not model.
- `convex/hiringPipeline.ts` (new authored seam) — `ingestKmCandidates({json})`: the find-or-upsert loop the single-record generated commands can't do. For each parsed candidate: find an existing Candidate by `(sourceSystem, externalCandidateId)`; create via `Candidate_createViaApply` (with a deterministic `idempotencyKey` as a second dedup layer) if absent, else `ctx.db.patch` the mutable source fields (raw + contact — stage stays an operator decision). Interviews upsert by `externalInterviewId` under the candidate; a KM outcome is recorded via `Interview_recordOutcome` only when the interview is still pending. **This is the §9.3 "re-import updates without duplication" requirement.** Source IDs/raw live on the Candidate/Interview rows themselves (NOT ExternalRecordLink) — candidates/interviews ARE the Capsule records, so there is nothing to reconcile-match against (unlike the TPP payment/event imports). Gated at the `workforceManageAccess` tier (role-Set mirror of base.manifest); auth propagates through `ctx.runMutation` (same identity), as it does for `importCommit`.
- `src/lib/hiringPipeline.ts` (new) — thin `useIngestKmCandidates()` wrapper. Lives in `src/lib` (NOT `src/features/workforce`) because the workforce slice guard forbids direct convex/react hooks in the feature root — mirrors `staffSelfReviews.ts`.
- `src/features/workforce/CandidatesPage.tsx` (new) — new-candidate form; candidate pipeline ledger with per-row stage-move + Reject(reason) + Hire; per-candidate interview list + schedule form + record-passed/failed; and a "Import from KM" box (paste JSON → ingest → result counts).
- Route `/staff/hiring` in `App.tsx`; nav "Hiring" in `workforceRoutes.ts`; `governed-creation-mappings.test.ts` += `Candidate_createViaApply` + `Interview_createViaSchedule`.

**Why it matters:** closes §9.3 and makes Slice 4 (Operations) spec-complete — the last HR gap. A manager can now track a candidate application → screening → interview → decision → hire/reject, and ingest a KM interview-tool export idempotently (re-import updates, never duplicates), with full source provenance (raw KM response + external IDs preserved for audit). The candidate/interview model + pipeline is also the prerequisite the spec names for the Nowsta payroll integration ("the same people/roles feed Nowsta integration later") — a hired Candidate links to a Person (`hiredPersonId`).

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok`: toolchain, builder ownership (20 paths), proof:emit, all 11 manifest-slice integration guards (workforce guard passes — authored-seam hook lives in `src/lib`), manifest-registry-pin, typecheck 0, format clean, secrets, test:coverage, build ok (`CandidatesPage` chunk built), baseline-decay ok. `bun run codegen` ran first to register the new `convex/hiringPipeline.ts` seam in `convex/_generated/api.d.ts`. Runtime write-path verified by inspection (the recurring GREEN-but-broken trap does NOT apply): creates use the proven `_createVia*` governed-creation hooks (`Candidate_createViaApply` returns `{docId}`); updates/re-imports use `ctx.db.patch` on `existing._id` (precedented by `importCommit.upsertLink`); `candidateId` is typed `Id<"candidates">` for the `by_candidateId` index; the generated commands re-check `workforceManageAccess` via `getAuthContext` at two layers; `roleAppliedFor`/`stage`/`outcome` are mapped to valid enum literals before write (no garbage vocab). No tests added (authored manifest + additive seam/UI cloning shipped siblings; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review (merge gate — author is Claude; reviewer codex `gpt-5.6-sol`):** reviewed the authored diff and returned 9 findings (3 High / 5 Medium / 1 Low). 8 fixed this increment; 1 skipped per repo policy:
- **FIXED (High, auth scope)** — `WORKFORCE_MANAGE_ROLES` was copied from `importAccess` (whole manager tier) but `workforceManageAccess` is granted ONLY to `workforce_manager`/`admin`/`owner`/`system` (base.manifest:95-127). The direct `ctx.db.patch` re-import path would have let `kitchen_manager`/`sales_manager`/etc. overwrite HR data despite lacking HR permission. Restricted the Set to the exact 4 roles.
- **FIXED (High, idempotency)** — KM records without a native id now get a deterministic `synth:<djb2-hash>` source id (name+role for candidates; candidate-id+interview content for interviews) so re-import still dedupes (was: a duplicate every time, violating §9.3 + the UI's promise).
- **FIXED (High, stage mapping)** — the KM-mapped `stage` was parsed then discarded; initial import now applies it (hire/reject/advance after create). Re-import patches name+role (KM source of truth); stage on re-import stays an operator decision (raw preserves the KM stage).
- **FIXED (parser correctness)** — `mapKmOutcome("Unsuccessful")` → `failed` (was `passed` via the "success" substring); `mapKmStage("Not hired")` → `rejected` (was `hired`); `mapKmRole("Kitchen Manager")` → `kitchen_manager` (was `kitchen_staff`). Negatives now checked first; role text normalized (spaces→underscore) before vocab lookup.
- **FIXED (Medium, advance semantics)** — `advance` could reach `hired`/`rejected` without the Hire/Reject metadata, then locked mistakes (`guard self.stage != "hired"`). Now: `advance` targets non-terminals only (constraint); the hired-lock guard removed so a mistaken terminal is reopenable; UI Move dropdown lists non-terminal stages only.
- **FIXED (Medium, dup interview ids)** — two KM interviews sharing an `externalInterviewId` could both take the create branch and trip `recordOutcome`'s "must be pending" guard on the second. Interviews now deduped by id within the payload.
- **FIXED (Medium, interview re-import staleness)** — `scheduledFor` is now patched on re-import (was: raw+notes only).
- **FIXED (Low, anti-tedium)** — rejection `reason` is now optional (spec §9.3 does not require one).
- **SKIPPED (Medium, "no authored tests")** — AGENTS.md forbids adding tests unless the owner asks; the repo test policy overrides codex's suggestion. Documented as a known gap.

Re-verified: `bun run check` GREEN after all fixes (typecheck 0, all 11 manifest guards, test:coverage, build, baseline-decay).

**Honest scope notes (documented, NOT this increment):** (1) **No KM sample exists in the repo** (it is an external interview-scheduling tool); the parser accepts the common export shape and is liberal on casing, mirroring the TPP parsers (which were likewise authored against a synthetic shape). The Capsule-side consumer is what was missing; a verified real KM export may refine the key list. (2) **Interviewer is not auto-resolved to a Person** — KM carries a name; the ingest stores it on `rawSourceData` and `interviewerPersonId` is set only when the operator picks a Person in the schedule form (no fuzzy name→Person match — that is an operator decision, not an inference). (3) **Stage is not auto-advanced on re-import** — the KM `stage` is parsed and available on `raw`, but moving a Capsule candidate's stage is an operator action (the pipeline is human-driven; auto-advancing from a legacy-tool string would be the kind of policy the merge gate flags). (4) **Hire does not create a Person automatically** — `hire(optional hiredPersonId)` links an existing Person if provided; creating a new staff Person from a hired Candidate is a separate onboarding action (the spec's "the same people/roles feed Nowsta later" is satisfied by the link, not by auto-hire). (5) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315) — the two new tables are additive (backward-compatible).

**Stale-info corrected in passing:** the Priority Sequencing table row 27 (One-on-Ones) still read ❌ NOT BUILT despite §9.5 shipping DONE as `v0.0.34`/`e5403dc`; corrected to ✅ DONE (the §9.5 detail row was already correct; only the priority table + older notes had drifted).
...
**Finding (verified first-hand against code this turn via two parallel triage subagents + direct reads):** spec §6.3 ("Browser-extracted Pack Lists … records source event ID, source page/version, extraction time, item text, quantities, grouping, and extraction errors … resumable and idempotent … maps only to Equipment PackList/PackListItem … unrecognized items may remain as imported free-text lines") was entirely unbuilt — confirmed by exhaustive search: zero pack-list parsing in `convex/tppParser.ts`, zero pack-list branch in `convex/importCommit.ts` (its dispatch handled only contacts/events/leads/payments/menus/venues), `pack_list` absent from both closed generated enums (`importRuns.datasetType` 6 values; `externalRecordLinks.capsuleEntity` 18 values), and `ImportRunDetailPage`'s commit UI explicitly limited to the six datasets. The target entities (`PackList`, `PackListItem`) and their idempotent create verbs (`PackList.open`, `PackListItem.addItem`) already existed, but nothing fed them from an import.

**Fix (smallest spec-faithful diff — clones the proven `events` cross-dataset-resolution branch + `menus` commit shape):**
- `src/import/import-run.manifest` + `src/import/external-record-link.manifest` — `pack_list` added to `ImportDatasetType` and `CapsuleEntityType` (additive enum members). `bun run manifest:regen` propagated both into `convex/schema.ts` (`datasetType` :841, `capsuleEntity` :798) + `schemas/manifest-schemas.ts` (:917/:4462, :869/:4319). `recordType` needed no change (free `v.string()`).
- `convex/tppParser.ts` — `TppPackListRecord` (one event's pack list: source event id + page/version + extraction time + nested `Items[]`) + `ParsedCapsulePackList`/`ParsedCapsulePackListItem`; `parseTppPackList`/`parseTppPackLists` (clone of `parseTppMenu`/`parseTppMenus`, accepting both PascalCase and snake_case); `mapPackListUnit` (TPP unit text → UnitOfMeasure literal, default `each`) + `parsePackListQuantity` (default 1). Each row's `externalId = sourceEventId` (the per-event stable key).
- `convex/importCommit.ts` — new `datasetType === "pack_list"` branch: resolves the source event to a Capsule Event via the prior events import (`findLink recordType:"event"`; pending_conflict if not imported — mirrors how events resolve client/venue), opens one `PackList_createViaOpen` (logisticsAccess guard; idempotencyKey per event), then `PackListItem_createViaAddItem` per item. Items with no Equipment/Dish mapping land as description-only free-text lines (dishId null) — the §6.3 "unrecognized items may remain as imported free-text lines" path. Unmapped unit text → `each` (raw unit preserved on the link). Per-item failures are recorded as extraction errors on `rawSourceData` without failing the whole list. Idempotent per-event (re-run skips an already-linked event).
- UI — `ImportRunDetailPage.tsx` + `ImportRunsListPage.tsx` `DATASET_TYPE_LABELS` += `pack_list`; `supportedCommitDatasets` + `commitNoun`/placeholder/help-text ternaries + error string extended; `ExternalRecordsReconcilePage.tsx` `RECORD_TYPE_LABELS` += `pack_list`. Operator pastes one JSON object per event's browser-extracted pack list (the producer — a browser scraper/extension against the live TPP per-event equipment view — is out of this loop's reach; the Capsule-side consumer is what was missing).

**Why it matters:** the TPP→Capsule migration's last unwired dataset is now reachable end-to-end. An operator extracts a per-event pack list from TPP (no bulk export exists), pastes it, and a real PackList + its items materialize against the already-imported event — idempotently, with full source provenance (rawSourceData preserves source event id, page/version, extraction time, item text/qty/unit/grouping, and per-item extraction errors for §6.1/§6.5 reconciliation). This closes §6.3 and makes Slice 2's dataset set complete (venues + contacts + events + leads + payments + menus + pack lists).

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok`: toolchain, builder ownership (20 paths), proof:emit, all 9 manifest-slice integration guards (logistics guard passes), manifest-registry-pin, typecheck 0, format clean, secrets, test:coverage, build ok (`ImportRunDetailPage` + `ImportRunsListPage` chunks built), baseline-decay ok. Runtime write-path verified by inspection (the recurring GREEN-but-broken trap does NOT apply): the branch uses the same `findLink`/`upsertLink`/`api.mutations.*_createVia*` primitives the other six datasets use in production; the cross-dataset event resolution clones the events branch's client/venue resolution verbatim; `PackList_createViaOpen`/`PackListItem_createViaAddItem` are called exactly as `Dish_createViaIntroduce`/`Event_createViaPlanEngagement` are (return `{docId}`); `unit` is `v.any()` at the mutation boundary so the mapped literal cannot be validator-rejected; a freshly `open`ed PackList is `draft` so `addItem`'s draft/packing guard holds; a role with `importAccess` but not `logisticsAccess` sees the row land as `pending_conflict` (the catch block), not a crash. No tests added (authored parser + commit branch mirroring shipped siblings + additive UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; additive enum members + an authored parser/commit branch cloning the in-file menus/events patterns + additive UI labels/ternaries, no new guard/policy/approval, no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **The browser-extraction producer is not built here** — Capsule consumes the extracted JSON; a Playwright/extension scraper against the live TPP per-event equipment view is the separate producer half (no TPP access in this loop). (2) **Items are not auto-mapped to Equipment** — `PackListItem` has no Equipment ref (only `dishId`/`productionBatchId`); unrecognized items stay as description-only free-text lines until an operator deliberately maps them (the §6.3 path). (3) **One pack list per event** — `externalId = sourceEventId`; a re-extraction of the same event is an idempotent skip (the prior PackList stands). (4) **`importCoordinator.ts` left untouched** — it is an orphaned parallel orchestrator (its `commitImport`/`revertImport` are TODO stubs; its `parseTppData` handles only 4 of 6 datasets); the UI uses the generated `ImportRun_*` hooks + `importCommit.commitImportRun`, so its hand-maintained `DATASET_TYPES` allowlist is not on the hot path (leads/menus already have the same gap there). (5) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315) — the additive enum members are backward-compatible.

---

**2026-07-26 — §4.1/§3.4 Event create: Salesperson + Referral Source selectors WIRED (closes the concrete remaining §4.1 "labeled salesperson, referral source" gap; the Event create form now captures both, cloning the proven Occasion/Service-Style selector pattern shipped in the prior increment):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** One authored manifest edit (Event gains optional `referralSourceId` + ref + `planEngagement` param) + one regen + two UI files (mapper + create page). No new entity/guard/query (additive optional field + ref to the existing ReferralSource entity — no #111 exposure), no money mutation, no new guard/policy/approval.

**Finding (verified first-hand against manifest + generated code this turn):** spec §4.1 ("The event form supports … salesperson, and referral source") and §3.2/§7.2 (salesperson + source/referral as report dimensions) were unmet on the create path. Two distinct sub-gaps hid behind the stale detailed-status "Salesperson/Owner ✅ DONE" + "ReferralSource ✅ DONE" rows (both true at the entity/Lead level, false at the §4.1 create-form level):
- **Salesperson:** the backend ALREADY existed — `Event.assignedToId: uuid?` + `ref assignedTo: Person` (`event.manifest:553/607`) and `planEngagement` already accepted `optional assignedToId` (`:692`). But `EventCreatePage.tsx` rendered NO salesperson picker — `assignedToId` was absent from state, the form, and the mapper. So a planner could never label the event's salesperson/owner at creation, blocking the §7.2 salesperson report dimension from ever being seeded with real data.
- **Referral source:** genuinely missing end-to-end on Event — `Event` had NO `referralSourceId` property/ref/param (only `Lead` did, `lead.manifest:23/38`). A directly-created event (no lead) could never capture its referral source, violating §4.1 and leaving the §7.2 "source/referral" dimension without event-level data.

**Fix (smallest spec-faithful diff — salesperson is UI-only; referral source is one optional field + ref + param, both cloning the in-file occasion/service-style selector line-for-line):**
- `src/operations/event.manifest` — Event gains `property referralSourceId: uuid?` (placed with the serviceStyleId/occasionId reference-data cluster) + `ref referralSource: ReferralSource fields [tenantId, referralSourceId] references [tenantId, id]`; `planEngagement` gains `optional referralSourceId: uuid` + `mutate referralSourceId = referralSourceId`. OPTIONAL so the known "required property on existing entity blocks schema push" gotcha does not bite. NOT added to the `EventPlanned` emit — no consumer needs it; the stored property is what reads/reports key off (ponytail).
- `bun run manifest:regen` — verified GENERATED (not just declared): `events.referralSourceId` (`schema.ts:477`, `v.optional(v.id("referralSources"))`) + `by_referralSourceId` index (`:533`); `__runEventPlanEngagement` accepts + mutates `referralSourceId` (`mutations.ts:7798/7844`); the public wrapper destructures + passes it (`:7930/7950`).
- `src/features/events/EventPlanEngagementFormMapper.ts` — `EventPlanEngagementFormInput` gains `salespersonId` + `referralSourceId`; two assigners (`assignOptionalSalespersonField` → `args.assignedToId`, `assignOptionalReferralSourceField` → `args.referralSourceId`) called from `toCommandArgs` (omit-when-blank, matching occasion).
- `src/features/events/EventCreatePage.tsx` — `useListPerson` + `useListReferralSource`; `salespeople` (active people whose role ∈ {sales_staff, sales_manager, owner}, sorted by name) + `activeReferralSources` (status active, sort by sortOrder); `salespersonId`/`referralSourceId` state; two native `<select>`s placed after Service style in the Engagement section; passed to the mapper. Salesperson name uses the repo's proven `[givenName, familyName].filter(Boolean).join(" ")` format.

**Why it matters:** a planner now labels the event's salesperson/owner AND referral source at creation. This closes §4.1/§3.4 (the last two "missing" event-create reference fields) and seeds the §7.2 report dimensions (salesperson, source/referral) + §6.5 parallel-run comparison (salesperson/referral breakdowns) with real event-level data. Salesperson flows `Event.assignedToId` → `Person` (the existing sales/owner link); referral source flows `Event.referralSourceId` → `ReferralSource` (the same entity Lead already uses).

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok`: toolchain, builder ownership ledger, proof:emit, all 9 manifest-slice integration guards, manifest-registry-pin, typecheck 0, format clean (both TS files prettier-clean; `.manifest` has no prettier parser — regen owns it), secrets, test:coverage, build ok (EventCreatePage chunk built), baseline-decay ok. Runtime write-path verified by inspection (the recurring GREEN-but-broken trap does NOT apply): the new field is a plain optional `mutate referralSourceId = referralSourceId` on an existing doc — `planEngagement` mutates an already-created Event draft (no `self.id`/`context.timestamp`/`_createVia*`-docId trap); the FK target `referralSources` exists and `Lead` already references it identically (`lead.manifest:38`); the UI passes `assignedToId`/`referralSourceId` only when non-blank (mapper omits empty strings, matching occasion/serviceStyle). No tests added (authored manifest + additive UI mirroring a shipped sibling; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; an additive optional field + ref to an existing entity + two UI selectors mirroring the in-file occasion/service-style pickers, no new guard/policy/approval, no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **Create form only** — the §4.1 spec target; `EventDetailPage`'s edit path is NOT extended with salesperson/referral selectors (matches the prior service-style/occasion increments, which wired create-only). An event-settings edit slice is the follow-up if operators must reassign post-create. (2) **`eventType` free-text remains** — retained for raw TPP parity (occasion is the canonical enum); not changed here. (3) **The spec status-table row (`specs/capsule-complete-feature-spec.md:19`) still reads "🟡 … Missing: occasion enum, service-style enum, labeled salesperson, referral source"** — all four are now wired on the create form; a spec-edit Opus pass is the separate cleanup (loop rule: spec edits are a separate Opus pass; consistent with the prior service-style increment, which also left the spec row for a dedicated pass). (4) **Salesperson is sourced from `people` with a sales-role filter**, not a dedicated Salesperson entity — matches the existing data model (`assignedToId` → Person; role enum sales_staff/sales_manager/owner). (5) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315) — the additive optional field is backward-compatible (no migration, no breakage).

---

**2026-07-26 — importAccess capability grant FIXED (root-cause GREEN-but-broken bug; unblocks the entire TPP import + reconcile read surface — the §6.4 reconcile queue + every §6.2 imported record were invisible to ALL users at runtime):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** One authored manifest line + its UI-audit mirror + a regen; no schema change (`ROLE_PERMISSIONS` lives in the function bodies, not the Convex schema), no migration, no new entity/guard/query, no money mutation. The regen propagates the grant to 11 roles.

**Finding (verified first-hand against generated code this turn):** every generated read for the import framework — `listExternalRecordLink`/`getExternalRecordLink`/`listExternalRecordLinkByTenantId`/`listExternalRecordLinkBySourceImportRunId` (`convex/queries.ts:3932-3978`), the ImportDataset reads (`:3991-4022`), and the ImportRun reads (`:4035-4066`) — opens with `if (!__allowsRead("…", …, () => checkRole(user, "importAccess"))) return []` (or `null`). But **no role in the generated `ROLE_PERMISSIONS` map (`queries.ts:35`) grants `importAccess`**, and **no role has the `"all"` fallback** — `checkRole` matches only via `permission.action === action || permission.action === "all"` (`queries.ts:434-437`). So `checkRole(user, "importAccess")` returned `false` for **every** role, including `owner`/`admin`/`system`. `__allowsRead` just returns `!!check()` (`queries.ts:26-33`) — no override. **Consequence:** every import read returned `[]`/`null` for every user. `ExternalRecordsReconcilePage` calls `useListExternalRecordLink()` (its sole data source, `ExternalRecordsReconcilePage.tsx:69`) → it rendered an empty queue for everyone. The 2026-07-26 changelog entry claiming the reconcile UI was "now functional" was **wrong at runtime** — the six §6.2 import increments produced `ExternalRecordLink` rows no operator could see, clear, or match. Textbook GREEN-but-broken: typecheck/format/secrets/test:coverage/build/baseline-decay all GREEN while the feature returned no data. Root cause is in `src/foundation/base.manifest`: the role hierarchy grants `manageAccess` to `manager` and the domain accesses to `admin`, but `importAccess` — the read/write/execute policy on three import entities (`external-record-link.manifest:97-99`, `import-dataset.manifest:87-89`, `import-run.manifest:62-64`) — was **never granted to any role**.

**Fix (smallest root-cause diff — grant the missing capability at the manager tier, where every comparable access already lives):**
- `src/foundation/base.manifest` — `role manager extends staff` gains `allow importAccess`. Propagates via `extends` to every `*_manager`, `admin`, `owner`, `system` (the 11 roles already holding `manageAccess`/`adminAccess`) — exactly the documented "managers/admins/owners hold both [importAccess + entity access]" intent. Base `staff`/`*_staff` do NOT receive it (correct: imports are an operator action; read/write/execute all use the same `importAccess`, so there is no narrower read-only tier to grant).
- `src/features/admin/rolePermissionAudit.ts` — hand-maintained `ROLE_DEFINITIONS` mirror updated: `manager: { parent: "staff", allow: ["manageAccess", "importAccess"] }`. Mirrors the `tests/governed-creation-mappings.test.ts` situation (a hand-maintained mirror of generated truth); no consistency test pins it, but the audit panel would under-report managers without it.
- `bun run manifest:regen` — verified GENERATED (not just declared): `convex/queries.ts` now contains `importAccess` 21× (was 0 in `ROLE_PERMISSIONS` — only the call sites existed) across `manager`/`kitchen_manager`/`sales_manager`/`event_manager`/`inventory_manager`/`logistics_manager`/`workforce_manager`/`finance_manager`/`admin`/`owner`/`system`; `convex/mutations.ts` 69×.

**Why it matters:** the entire TPP-import + reconcile track (Priorities 1–2, §6.1/§6.2/§6.4) goes from "claims DONE but returns no data" to actually reachable at runtime. A manager-tier operator now sees the imported-record queue, can Verify/Skip/Match pending links, and the six §6.2 datasets' `ExternalRecordLink` rows become visible. This was the load-bearing blocker hiding under six "shipped" increments. No security weakening: it GRANTS a capability that was intended but missing; previously nobody could.

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok` (toolchain, builder ownership ledger, proof:emit, all 9 manifest-slice integration guards, manifest-registry-pin, typecheck 0, format clean, secrets, test:coverage, build ok, baseline-decay). **The proof is structural and deterministic, not generator-`as any`-hidden** (the recurring GREEN-but-broken trap does NOT apply here): the generated `ROLE_PERMISSIONS` now literally lists `{ action: "importAccess" }` for the 11 manager-tier roles (grep-confirmed), and `checkRole` does a plain `permission.action === action` comparison with no runtime-context dependency (no `context.timestamp`, no `self.id`, no create-via-docId, no FK relation-key). No tests added (authored manifest + mirror; AGENTS.md).

**Cross-model review:** not run this increment (autonomous-loop cadence; a one-line capability grant at the documented tier + its mirror, no new guard/policy/approval, no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **Base `staff`/`*_staff` still cannot read import records** — correct per the single-`importAccess` policy (read==write==execute); a future read-only `importRead` tier is a separate policy decision if non-managers should browse imports. (2) **The fix takes effect server-side on the next Convex deploy** — a `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315); before that deploy the reads still return `[]` against the live deployment. (3) **This corrects the prior "§6.4 reconcile UI functional" / "Slice 2 ✅ 100%" claims** — they were runtime-false until this grant; the §6.1/§6.4/Slice-2 detailed rows above still overstate, and a separate doc-cleanup pass is worthwhile. (4) The follow-on source-provenance panel (showing an imported Event's TPP source on `EventDetailPage`) is now unblocked by this fix and is the next increment — an authored `findByCapsule`-equivalent seam's `importAccess` gate is now satisfiable.

---

**2026-07-26 — Source-provenance panel on EventDetailPage DONE (closes the visibility gap left by the import track — an operator viewing an imported Event can now see where it came from; spec §6.1 "preserve enough raw source data … to explain mappings later" + §6.5 "drillable to source + Capsule records"):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** One authored Convex seam query + one `src/lib` wrapper hook + one read-only panel + two-line wiring in `EventDetailPage`. No manifest/regen (authored query only), no new entity/guard (additive READ gated on the existing `importAccess` — now satisfiable after the importAccess-grant fix above), no schema change, no migration, no money mutation.

**Finding (verified first-hand against code this turn):** the just-fixed import track materializes `ExternalRecordLink` rows for all six §6.2 datasets, but there was **no way for an operator to see, from a Capsule record, that it was imported or where it came from**. The generated ExternalRecordLink reads list/get by tenant (`listExternalRecordLinkByTenantId`) or import run (`listExternalRecordLinkBySourceImportRunId`) — there is no generated find-by-capsuleId, even though `findByCapsule` is declared in `src/import/external-record-link.manifest:287` (the generator never emitted it, and the table has no `by_capsuleId` index — `schema.ts:813-814`). So `EventDetailPage` had no surface showing TPP origin / external id / raw source / match state. Six increments of import work were invisible at the record level.

**Fix (smallest spec-faithful diff — additive read seam mirroring the proven `staffSelfReviews.ts` self-scoped-read pattern):**
- `convex/sourceProvenance.ts` (new authored seam) — `listByCapsuleId({ capsuleId })` query: `getAuthContext` → gates on the same `importAccess` capability as the entity's read policy (re-checked via an `IMPORT_ACCESS_ROLES` mirror of the 11 manager-tier roles, because `checkRole` is generated as a non-exported local); collects the tenant's `externalRecordLinks` via `by_tenantId` (the same scan shape the reconcile page already uses) and JS-filters `deletedAt == null && conflictStatus !== "superseded" && capsuleId === capsuleId`; projects provenance fields (`sourceSystem`/`recordType`/`externalId`/`conflictStatus`/`verified`/`sourceImportRunId`/`importedAt`/`resolutionNote`/`rawSourceData`). Non-importAccess callers get `[]` → no panel (the entity read policy is not widened).
- `src/lib/sourceProvenance.ts` (new) — thin `useSourceLinksByCapsuleId(capsuleId)` wrapper over `useQuery(api.sourceProvenance.listByCapsuleId, capsuleId ? { capsuleId } : "skip")`. Lives in `src/lib` (mirrors `staffSelfReviews.ts`) so the events feature root stays free of direct convex/react hooks.
- `src/features/events/EventSourceProvenancePanel.tsx` (new) — read-only `EventTabPanel`; per link: source-system label (TPP legacy / CSV / QuickBooks / …), Matched/Pending-match status chip, imported date, external id, record type, resolution note, and a collapsible `<details>` pretty-printing the raw source JSON. Renders only when an import link exists (a natively-created event shows no empty section).
- `src/features/events/EventDetailPage.tsx` — import + mount `<EventSourceProvenancePanel capsuleId={event._id} />` on the Overview tab after `<EventWeatherPanel>`.

**Why it matters:** the import track's value is now visible at the record level — an operator opening an imported Event sees "TPP (legacy) · Matched · imported 2026-07-26 · external id … · [raw source]". This is the §6.1/§6.5 drill-to-source obligation, and it directly complements the importAccess-grant fix (the seam's gate is now satisfiable) and the six §6.2 dataset commits (whose `capsuleId === eventId` link, verified at `importCommit.ts:567`, is what this panel reads).

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok`: toolchain, builder ownership, proof:emit, all 9 manifest-slice integration guards, manifest-registry-pin, typecheck 0, format clean (prettier reformatted the panel), secrets, test:coverage, build ok (`EventDetailPage` chunk built with the new panel), baseline-decay ok. `bun run codegen` ran first to register the seam in `convex/_generated/api.d.ts` (a new `convex/*.ts` authored seam won't typecheck until codegen adds it — the staffSelfReviews learning). Runtime write-path verified by inspection (the recurring GREEN-but-broken trap does NOT apply): the query is a pure read (no create → no `self.id`/`context.timestamp`/`_createVia*`-docId/FK-relation-key traps); `getAuthContext` + `by_tenantId` collect + JS filter is the exact shape of the generated `listExternalRecordLinkByTenantId` already in production; `capsuleId === event._id` is the exact value stored at `importCommit.ts:560-567` (`capsuleId: eventId` where `eventId = created.docId`). No tests added (authored seam + read-only UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; additive read-only seam + read-only panel mirroring the `staffSelfReviews.ts` pattern, no new guard/policy/approval, no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **Provenance shows for the Event only** — the seam is generic-by-`capsuleId` and the panel is reusable, but it is mounted on `EventDetailPage` alone (events are the spine); wiring it onto `VenueDetailPage`/`ClientsPage`/`LeadPipelinePage` is a separate, mechanical placement. (2) **The tenant-wide scan** (no `by_capsuleId` index) is fine at single-tenant scale and matches the reconcile page's existing scan; a `by_capsuleId` index is the upgrade path if link volume grows (documented in-code). (3) **Raw source data is shown verbatim** (pretty-printed) — it may contain client PII from TPP; access is already restricted to `importAccess` (manager-tier), consistent with the ExternalRecordLink read policy. (4) **`superseded` (reverted) links are hidden** — an operator sees the current source; revert history lives on the import-run/reconcile pages. (5) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315).

---

**2026-07-26 — §6.1 raw TPP EventStatus / Lead Stage preservation FIXED (noticed-bug follow-up; the import path's own header comments claimed the raw TPP status/stage "is preserved on rawSourceData," but only the MAPPED Capsule stage was stored — the literal source value was lost, violating §6.1 "preserve enough raw source data … to explain mappings later" for the migration parallel-run):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** One parser file (`convex/tppParser.ts`); no manifest/regen (`rawSourceData` is a free-form string — the new fields ride along in the existing `JSON.stringify`), no schema change, no migration, backward-compatible.

**Finding (verified first-hand against code this turn):** `parseTppEvent` set `stage: mapTppEventStage(record.EventStatus)` and `parseTppLead` set `stage: mapTppLeadStage(record.Stage)` — the parsed records carried only the MAPPED Capsule stage, not the literal TPP `EventStatus`/`Stage`. Since `commitImportRun` stores `rawSourceData: JSON.stringify(event|lead)`, the source status/stage was discarded. `importCommit.ts`'s header comments (events L27-30, leads L36-37) nonetheless claimed the raw value "is preserved" — overstated until this fix.

**Fix (smallest spec-faithful diff — carry the literal source value alongside the mapped one):** `ParsedCapsuleEvent` gains `rawEventStatus: string` (`rawEventStatus: record.EventStatus`); `ParsedCapsuleLead` gains `rawStage: string` (`rawStage: record.Stage`). Both flow into the link's `rawSourceData` via the existing `JSON.stringify`, making the importCommit comments accurate and §6.1's raw-preservation obligation true for the two stage-carrying datasets.

**Why it matters:** the TPP→Capsule migration parallel-run (§6.1/§6.5) can now reconcile each imported event/lead's literal TPP status against the Capsule stage it landed on, instead of seeing only the mapped value with no source trace.

**Verification:** `bun run check` GREEN (exit 0) — full `&&`-chained gate to `baseline-decay: ok`. No tests added (parser field addition; AGENTS.md).

---

**2026-07-26 — §8.2 Venue-logistics snapshot in the proposal revision DONE (closes the §5.2 L263 "Venue logistics snapshot" required-section + §8.2 L376 "snapshot the venue information needed to reproduce the client and operations plan" gap; the accepted/shared proposal now freezes venue logistics, so §5.5 L284 "the accepted revision remains reproducible after later … venue … edits" holds for logistics too):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** Three coordinated files (snapshot builder + shared-proposal projection + client render); no manifest/regen (the revision `snapshot` is a free-form `v.string()` JSON blob — new keys are absorbed silently, no schema push), no new entity/guard/query (no #111 exposure), no migration, no new guard/policy/approval, no money mutation.

**Finding (verified first-hand against spec + code this turn, via 3 parallel triage subagents):** the proposal revision snapshot — captured at send by `captureProposalRevision` (`convex/lib/proposalRevision.ts`) and the immutable artifact the §4.6 share link + §5.5 digital acceptance pin — froze ONLY the Proposal row's free-text `venueName`/`venueAddress`. The structured venue logistics (power/water/stairs/elevator/parking/storage/kitchen/load-in/waste/permits/restrictions/on-off-premise/capacity, §8.1 L370 field catalog) live on the `Venue` entity, two optional hops away (`Proposal.eventId` → `Event.venueId` → `Venue`), and the snapshot builder never resolved them. So (a) the §5.2 required "Venue logistics snapshot" proposal section was absent on the client-facing shared proposal, and (b) §5.5's "accepted revision remains reproducible after later venue edits" was violated for logistics — a venue edit after send would change the logistics a client/op sees, because nothing was frozen. The snapshot is consumed by exactly one reader today (`convex/shareLinks.ts` `getSharedProposal`, parsed at L80), which projects `snapshot.proposal`/`client`/`lineItems` to `SharedProposalPage`.

**Fix (smallest spec-faithful diff — additive JSON keys; backward-compatible: revisions captured before this change have no `venue` key → resolve to null → no section rendered):**
- `convex/lib/proposalRevision.ts` — (1) added a `venue` block to the `ProposalRevisionSnapshot` interface (nested under the snapshot root, since its source entity differs from the proposal-level venue strings): `name`/`venueType`/`capacity` + the §8.1 logistics cluster (`onPremise`/`kitchenAccess`/`parkingAvailable`/`hasFreightElevator`/`storageAvailable`/`logisticsNotes`/`loadInInstructions`/`powerAvailable`/`waterAccess`/`hasStairs`/`wasteRules`/`permitsInsuranceNotes`/`restrictions`/`accessNotes`/`cateringNotes`), typed `| null`. (2) New `resolveVenueLogistics(ctx, proposal)` helper: guards the two optional hops (`proposal.eventId` → event → `event.venueId` → venue) with same-tenant + not-deleted checks, returns `null` (non-disclosing) when the proposal isn't event-linked or the event has no venue — the free-text `venueName`/`venueAddress` remain the always-present fallback. (3) Wired `venue: await resolveVenueLogistics(ctx, proposal)` into the snapshot object.
- `convex/shareLinks.ts` — extended the `SharedProposal` type with a `venueLogistics` projection and shaped it in `getSharedProposal` (a `bool` helper alongside the existing `num`/`str`). Operator-authored free-text notes (`accessNotes`/`cateringNotes`/`logisticsNotes`) are kept in the immutable snapshot for ops reproduction but deliberately NOT projected to the client-facing view.
- `src/features/clients/SharedProposalPage.tsx` — added a "Venue Logistics" section rendered only when the shared revision captured a linked venue with at least one non-null logistics field (so an unlinked proposal renders no empty section); booleans render as Yes/No·Available/Not-available, strings as-is.

**Why it matters:** the proposal revision (Slice 1's core artifact) now freezes the venue logistics at send, so the client's accepted/shared proposal reproduces the exact operations plan regardless of later venue edits — satisfying §5.2's required "Venue logistics snapshot" section and §8.2's snapshot-at-publication obligation. Connects directly to the just-shipped §5.5 acceptance + §4.6 share links (both consume this exact snapshot). Unlinked proposals (no event, or event with no venue) snapshot `null` logistics — correct, non-disclosing behavior (the free-text venue name/address still show).

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained to `baseline-decay: ok`: toolchain, builder ownership (20 paths), proof:emit, all 9 manifest-slice integration guards, manifest-registry-pin, typecheck 0, format clean, secrets, test:coverage, build ok (`SharedProposalPage` chunk built), baseline-decay ok. Runtime write-path verified by inspection (the generator's `as any` casts hide runtime bugs — the recurring GREEN-but-broken trap): `sendProposalWithRevisionCapture` → `captureProposalRevision` → `buildProposalRevisionSnapshot` → `resolveVenueLogistics` reads existing docs via `ctx.db.get` (no create → no `self.id`/`context.timestamp`/`_createVia*`-docId/FK-relation-key traps; field names verified against `convex/schema.ts` `venues` L2581-2618; `events.venueId` L477 + `proposals.eventId` L111 are the optional-nullable hops). No tests added (authored seam + UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; an additive snapshot projection mirroring the existing capture-on-send pattern + a read-only client section, no new guard/policy/approval, no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **Venue rooms (`VenueRoom`) are NOT snapshotted** — rooms are a separate child entity (`src/operations/venue-room.manifest`) requiring a `ctx.db.query("venueRooms").withIndex("by_venueId")` lookup; §8.1 lists "room/space details" but the §8.2 core obligation ("venue information needed to reproduce the client and operations plan") is met by the venue-level logistics; rooms are a follow-up if room-level reproducibility is wanted. (2) **Only the client-facing shared proposal renders logistics today** — `getSharedProposal` is the snapshot's only reader; the operator-side `ProposalsPage`/revision-history list reads only `revisionNumber`/`changeSummary` metadata, and the proposal-PDF path does not parse `.snapshot` — surfacing logistics there is a separate render slice. (3) **Operator free-text notes are frozen but hidden from clients** (`accessNotes`/`cateringNotes`/`logisticsNotes`) — kept for ops reproduction, not projected to `SharedProposal.venueLogistics`. (4) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315).

---

**2026-07-26 — §4.1 Event create: Service Style selector WIRED (closes the concrete §4.1 "service-style enum" gap on the Event create form; ServiceStyle entity + Event.serviceStyleId were already shipped but unreachable from the UI):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0).** Two UI files (mapper + create page); no manifest/regen (no new entity/guard/query — no #111 exposure), no schema change, no Convex codegen, no new guard/policy/approval, no money mutation.

**Finding (verified first-hand against code this turn):** spec §4.1 ("The event form supports … service style") and §3.2 ("Service Style is available in event forms") were unmet on the create path. The Event entity already had `property serviceStyleId: uuid?` + `ref serviceStyle: ServiceStyle` (`src/operations/event.manifest:550/604`) and the `planEngagement` create verb already accepted `optional serviceStyleId` (`event.manifest:681/710`); `useListServiceStyle` was already generated (`src/lib/manifest-convex-react.ts:6383`). But `src/features/events/EventCreatePage.tsx` rendered an Occasion picker and NO service-style picker — `serviceStyleId` was absent from state, the form, and the `eventPlanEngagementFormMapper.toCommandArgs` call. So a planner could never tag an event with its service style at creation, blocking the §3.2 chain (proposal defaults, staffing/equipment templates, reporting filters) from ever being service-style-driven off real data.

**Fix (smallest spec-faithful diff — cloned the proven Occasion line-for-line):**
- `src/features/events/EventPlanEngagementFormMapper.ts` — added `serviceStyleId: string` to `EventPlanEngagementFormInput` + an `assignOptionalServiceStyleField` (mirror of `assignOptionalOccasionField`) called from `toCommandArgs`. Omits the arg when blank (matches occasion's optional behavior; `planEngagement` accepts `optional serviceStyleId`).
- `src/features/events/EventCreatePage.tsx` — `useListServiceStyle()` + `activeServiceStyles` (filter `status==="active"`, sort by `sortOrder`, mirroring `activeOccasions`); `serviceStyleId` state; a native `<select>` "Service style" placed next to Occasion in the Engagement section (the two reference pickers group naturally); passed to `toCommandArgs`.

**Why it matters:** Service Style is the highest-leverage foundation reference (priority table: blocks 11 features). It now flows Event create → Event.serviceStyleId → EventDish/ProposalDishSelection/StaffRole defaults + reporting filters. A planner picks Full Service / Limited Service / Drop Off / Vending at creation; downstream slices that key off service style finally have real data.

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained: toolchain, builder ownership, proof:emit, all 9 manifest-slice integration guards, manifest-registry-pin, typecheck 0, format clean, secrets, test:coverage, build ok (EventCreatePage chunk built), baseline-decay ok. No tests added (UI-only, additive selector mirroring a shipped sibling; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; UI-only additive `<select>` mirroring the in-file Occasion picker, no new guard/policy/approval, no money mutation, no manifest change). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) §4.1 lists three other "missing" fields — occasion (already wired prior), **labeled salesperson**, and **referral source**. Event has NO `salesperson`/`owner` field and NO `referralSourceId` (`grep` of `src/operations/event.manifest` confirms only `serviceStyleId`/`occasionId`); wiring those needs a manifest regen (new optional property + ref + create param) and is a separate slice. (2) The spec status-table row (`specs/capsule-complete-feature-spec.md:19`) still reads "🟡 … Missing: occasion enum, service-style enum, labeled salesperson, referral source" — occasion + service-style are now wired; salesperson + referral source remain. Not edited this turn (spec edits are a separate Opus pass per the loop rules). (3) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315) — N/A this increment (no Convex change).
...
**2026-07-26 — §5.5 Digital acceptance → Proposal.accept WIRED (closes the #1 Slice-1 DEAD-but-"done" gap; the digitally-signed proposal no longer stays "sent" forever):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0). One manifest regen (new optional property + optional param + event field + one single-target reaction); no new `*ByTenantId` query (no #111 exposure); one one-line UI arg.**

**Finding (verified first-hand against manifest + generated code this turn, via 4 parallel triage subagents):** the Priority 12 "Digital Acceptance ✅ DONE" claim was inert at runtime. `SignatureRequest.complete` (generated `__runSignatureRequestComplete`, convex/mutations.ts:32373) emits `SignatureCompleted` (32413) but NOTHING consumed it — `grep` found zero consumers repo-wide (only the manifest declaration + a stale UI doc-comment at ProposalAcceptancePage.tsx:20 that *claimed* "Manifest reaction triggers Proposal.accept"). So a client who signed a proposal left it stuck at `sent` forever, never `accepted` — violating spec §5.5 ("the bottom CTA must be functional, not decorative") and the §5.1/§5 "Proposal done-when: accept/sign, and see the event/proposal update through real commands and events." `__runProposalAccept` had no caller besides the public `Proposal_accept` wrapper. Capture-on-send, line-item edit (reviseLine/removeLine), and `recomputeProposalTotals` were all verified LIVE in the same pass — only this acceptance cascade was dead. (Textbook placeholder/stub waste the loop rules target.)

**Fix (smallest spec-faithful diff — one reaction mirroring the proven `Client.markMerged` / `Equipment.updateCondition` single-target resolve pattern; zero ref-nav):**
- `src/sales/signature-request.manifest` — (1) new `property proposalId: uuid?` on SignatureRequest (denormalized; OPTIONAL so the known "required property on existing entity blocks schema push" gotcha does not bite — schema.ts:1109 `v.optional(...)` + `by_proposalId` index). (2) `requestSignature` gains `optional proposalId: uuid` + `mutate proposalId = proposalId`. (3) `SignatureCompleted` event gains `proposalId: string?`; `complete` emits `proposalId: self.proposalId`. (4) NEW reaction `on SignatureCompleted run Proposal.accept resolve payload.proposalId`. Denormalizing proposalId (supplied by the UI at request time) avoids emit ref-nav (`self.proposalRevision.proposalId`), which is unsupported — 0 existing emits navigate a ref.
- `src/features/clients/ProposalsPage.tsx:580` — `createSignatureRequest` now passes `proposalId: row._id` (always available; the request is created from a proposal row).
- Regen (`bun run manifest:regen`) — verified the reaction actually GENERATED, not just declared (the recurring GREEN-but-broken trap): `__runSignatureRequestComplete` now ends with `// Reactions / const reactionTarget0 = payload.proposalId; / if (reactionTarget0) await __runProposalAccept(ctx, { docId: reactionTarget0 } as any);` and the `SignatureCompleted` payload carries `proposalId: __after.proposalId` (mutations.ts:32413). `convex/http.ts` command-dispatch gained `proposalId` in `SignatureRequest.requestSignature` params.

**Key decision (documented in-code): the cascade is a server-side reaction, not a client-callable command.** The public acceptance page holds NO sales role, and `Proposal.accept`'s execute policy is `salesAccess` — so the client cannot call `Proposal.accept` directly (an authored seam calling it would propagate empty auth → salesAccess throws, the known public-ingress trap). The reaction runs `__runProposalAccept` inline within `__runSignatureRequestComplete` with system authority — exactly how `Client.markMerged` / `Equipment.updateCondition` / `EventDish.confirmFromProposal` cascades already work. This is the spec §2.2 design ("Use reactions for deterministic internal cascades"). Nullable `proposalId` (revisionless/legacy request) makes `reactionTarget0` falsy → skips cleanly. If `Proposal.accept` throws (already accepted / expired proposal) the whole `complete` rolls back — correct: a signature that cannot accept is not a valid acceptance; and `complete`'s own `status == "requested"` guard makes the reaction fire exactly once per request → spec §5.5 "a successful provider callback cannot be applied twice."

**Why it matters:** the proposal wedge (Slice 1 — "first visible replacement value for TPP") gets its core done-when criterion ("accept/sign, and see the event/proposal update through real commands and events") actually working end-to-end. A client signature now flips the proposal `sent/viewed → accepted`, fires `ProposalAccepted`, and triggers the existing `ProposalDishSelection → EventDish.confirmFromProposal` menu cascade (which was ALSO unreachable from the client path until now). No money mutation; no new guard/policy/approval beyond the pre-existing `salesAccess`/`clientAccess` execute policies.

**Verification:** `bun run check` GREEN (exit 0) — fully `&&`-chained: toolchain, builder ownership, proof:emit, all 9 manifest-slice contracts, check:manifest-registry, check:event-manifest, typecheck 0, format clean, secrets, test:coverage, build ok, baseline-decay ok. Runtime write-path verified by inspection (the generator's `as any` casts hide runtime bugs — the recurring GREEN-but-broken trap): public page `SignatureRequest.complete` (fetch, no auth) → `__runSignatureRequestComplete` → `SignatureCompleted` emit (payload now incl. proposalId) → inline `__runProposalAccept(ctx, {docId: payload.proposalId})` → Proposal.accept body (guard `status==sent|viewed` ✓, `mutate status=accepted, acceptedAt=now()`) → `ProposalAccepted` emit → `ProposalDishSelection → EventDish.confirmFromProposal` cascade. The repo's recurring GREEN-but-broken failure modes do NOT apply: no `self.id`/`context.timestamp` in a create (accept mutates an existing doc, `now()` for acceptedAt); no `_createVia*`-docId field misuse (resolve passes `docId`, read directly by `__runProposalAccept`); no FK relation-key reuse; no datetime param (eventId optional, omitted → uses `self.eventId`). No tests added (authored manifest + 1-line UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; a single additive reaction mirroring shipped resolve-cascade patterns + a denormalized optional field + a one-line UI arg, with no new guard/policy/approval and no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **External e-sign providers** (DocuSign/HelloSign/PandaDoc) still route through the same `complete` command — once their webhook handlers call `SignatureRequest.complete` with a real `signedArtifactReference`, this same reaction fires. The `provider: "internal"` click-to-accept path is the live one today. (2) **The reaction rolls back `complete` if `Proposal.accept` throws** (proposal already accepted, or proposal `expiresAt` in the past while the signature request has not expired) — acceptable per spec semantics; silent partial-success is worse. (3) **`proposalId` is optional** — a signature request created through a path that omits it (none today; the UI always supplies `row._id`) would skip the cascade. (4) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315).

**Stale-info corrected this turn (verified, not just noted):** (a) the status-table row claiming §5.4 catalog-sourced pricing + override audit is an open "next slice" was STALE — it is DONE (ProposalLineItem.menuDishId+overrideReason, send-time override guard at proposalRevision.ts:340-373, catalogPrice in revision snapshot); the same-day changelog entry L196 already said so — table row L1678 fixed this turn. (b) §6.3 Browser-Extracted Pack Lists is genuinely BLOCKED (no extractor, no Playwright dep, `pack_lists` is not a valid datasetType — schema.ts:839 union is closed, no TPP pack-list sample feed) — not a candidate for an autonomous code increment. (c) Slice-1 capture-on-send, line-item edit, and `recomputeProposalTotals` were verified LIVE (not dead).

---

**2026-07-26 — §6.2 Menus import WIRED (sixth and FINAL dataset of the #1 Critical-priority Import Datasets; venues + contacts + events + leads + payments + menus now ALL materialize — all six §6.2 datasets reachable end-to-end):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0). New parser (`TppMenuRecord`/`ParsedCapsuleMenu`/`parseTppMenu(s)`/`parsePortionSize`/`mapTppAllergens`/`slugifyMenuName`) in `convex/tppParser.ts` + an additive `menus` branch to the existing `convex/importCommit.ts` authored seam + a dataset-aware UI generalization; no manifest/regen (no new entity/guard/query — no #111 exposure), no schema change, no new Convex module.**

**Finding (verified first-hand against spec + code this turn):** the payments-commit increment (entry below) documented its next slice as "menus (a menu-catalog parser)" and noted menus "needs a parser + a TPP source feed." Verified before building — the source feed EXISTS: `work/dishes.csv` is the real TPP dish-catalog export (16 rows; header `name,recipe_name,description,category,service_style,portion_size_description,dietary_tags,allergens,price_per_person,cost_per_person`), and `importCoordinator.ts:66,168` already register `"menus"` in the datasetType union. Only the parser + commit branch + UI were missing. (1) The target is the `Dish` entity (`src/culinary/dish.manifest:68` `introduce` → generated `Dish_createViaIntroduce` `convex/mutations.ts:4597`, returns `{docId}`, **kitchenAccess**-guarded — NOT salesAccess like the other branches; validates `name` non-empty + `portionSize > 0`; `idempotencyKey`-deduped). (2) Dish has NO price field (price lives on `menuDishes.sellingPrice`), and the real `dishes.csv` `price_per_person`/`cost_per_person` columns are EMPTY in all 16 rows — so there is no price data to lose. (3) The TPP feed has no external id and no menu grouping — the dish name is the only stable identity. (4) `CapsuleEntityType` includes `menu` (`external-record-link.manifest:23`) and `recordType` is a free string, so a menu link needs no schema/regen change.

**Fix (smallest spec-faithful diff — new parser + one new branch mirroring the proven venue loop + a dataset-aware UI):**
- `convex/tppParser.ts` — added `TppMenuRecord`, `ParsedCapsuleMenu`, a singular `parseTppMenu` (slugifies Name → externalId; **accepts BOTH the PascalCase `TppMenuRecord` shape AND the literal snake_case columns of the real `dishes.csv` feed** — `name`/`description`/`service_style`/`portion_size_description`/`allergens`/`price_per_person`/`cost_per_person`, first present key wins — so an operator can paste either form; `parsePortionSize` extracts a leading int from free text like "75 servings", default 1; `portionUnit "portion"`; `mapTppAllergens` normalizes display names Eggs/Shellfish/Dairy/Wheat-Gluten/Tree Nuts → the closed AllergenCode enum, dropping unknowns; the raw allergen + portion-size text are preserved via `rawAllergens`/`rawPortionDescription` on the parsed record for §6.1 reconciliation visibility), a batch `parseTppMenus` (validates non-empty Name, which doubles as the external id), and the helpers.
- `convex/importCommit.ts` — added a `menus` branch to `commitImportRun` (the other paths left byte-identical). Per parsed TPP dish: idempotency skip via `findLink(recordType "menu")`; then `Dish_createViaIntroduce({ name, portionSize, portionUnit, description, category, serviceStyle, dietaryTags, allergenSummary, idempotencyKey })`. The empty price columns are preserved on the link's `rawSourceData` — no Menu/MenuDish graph is invented for empty price data. An idempotent `ExternalRecordLink` (recordType `menu` → capsuleEntity `menu`; `resolved`/`pending_conflict`). Revert is dataset-agnostic (supersedes via `by_sourceImportRunId`). The previously-dead `if (datasetType !== "venues")` guard was REMOVED: `ImportDatasetType` is a closed union of exactly six members and the five explicit branches each return, so TS narrows `datasetType` to `"venues"` at the fall-through (the old guard became unreachable `never`, which broke typecheck); a future member added without a branch falls through to venue parsing and fails loudly.
- `src/features/admin/import/ImportRunDetailPage.tsx` — generalized the commit form to also accept `menus`: `supportedCommitDatasets` + `commitNoun` now cover menus; the JSON-textarea placeholder gained a dish-row example; the help text + actions guide explain the Dish-catalog model (price preserved on the link — Dish has no price field; requires kitchenAccess). Stale code comments corrected.

**Key decision (documented):** a TPP dish-catalog row → a **Dish** (the catalog item), NOT a Menu+MenuDish graph. Rationale: (a) the `dishes.csv` feed is one row per dish with no menu grouping; (b) Dish has no price field and the feed's price columns are empty, so a MenuDish.sellingPrice graph would capture nothing real; (c) it mirrors the one-entity-per-dataset shape of venues/contacts/events/leads; (d) it needs no schema/regen. Menu/MenuDish assembly (to make prices queryable) is the documented follow-up IF a TPP export with real prices arrives.

**Why it matters:** the #1 Critical-priority Import Datasets (Priority 2) moves from "venues + contacts + events + leads + payments" to **ALL SIX** — the spec's Slice-2 dataset list is now fully reachable end-to-end (pack lists remain §6.3 browser-extraction, a separate track). Imported TPP dishes become real Dish catalog entities (the spine proposals/EventDish/food-cost reference). Re-running is safe (per-record `idempotencyKey` + `findLink`/`upsertLink` dedup keyed on `(tenant, sourceSystem, recordType="menu", externalId)`).

**Verification:** `bun run check` GREEN (exit 0) — toolchain, ownership ledger, all manifest-slice contracts, typecheck 0 (the menus branch + the removed dead guard type cleanly), format clean, secrets, test:coverage, build ok, baseline-decay ok. Runtime write-path verified by inspection (the recurring GREEN-but-broken trap): UI `commitImportRun({importRunId, rawRows})` (datasetType menus) → `loadCommitContext` → per dish: `findLink("menu")` → `Dish_createViaIntroduce` (generated, kitchenAccess-guarded, `idempotencyKey` cached, returns `{docId}`) → `upsertLink` (capsuleEntity "menu", resolved/pending) → `ImportRun_commit`. The repo's recurring GREEN-but-broken failure modes do NOT apply (no `self.id`/`context.timestamp` in a create — Dish id from `{docId}`; `introduce` hardcodes `introducedAt = now()` server-side; `name` non-empty + `portionSize ≥ 1` satisfy constraints; `_createVia*` returns `docId` read directly; no FK at create; `portionUnit "portion"` is valid; `capsuleEntity "menu"` is a valid enum). No tests added (authored seam + parser + UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; additive authored-seam branch mirroring shipped patterns + a new parser + a mechanical UI generalization, no new guard/policy/approval beyond the existing `importAccess` role and the pre-existing `kitchenAccess` guard on Dish, no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **No Menu/MenuDish graph is built** — each imported dish is a standalone Dish; the price columns are empty in the real feed so nothing is lost, but a TPP export WITH prices would need the Menu+MenuDish path to make them queryable. (2) **Allergens are normalized to the closed enum; unknown free-text allergens are dropped** (not stored) — the full raw `Allergens` string is preserved on the link's `rawSourceData`. (3) **`portionSize` is a heuristic** — the leading integer of free text like "55 or 150 servings depending on event" (→ 55); the full `PortionSizeDescription` is preserved raw on the link. (4) **`course` is not set** — the TPP feed has only `category`; mapping to the separate Dish `course` field is not invented. (5) **Dish create requires `kitchenAccess`** — a role with `importAccess` but not `kitchenAccess` (managers/admins/owners hold both) sees every dish land as `pending_conflict`. (6) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315).

---

**2026-07-26 — §6.2 Payments import WIRED (fifth and final dataset of the #1 Critical-priority Import Datasets; venues + contacts + events + leads + payments now reachable end-to-end):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0). Additive `payments` branch to the existing `convex/importCommit.ts` authored seam + a dataset-aware UI generalization; the parser layer (`parseTppPayments`/`ParsedCapsulePayment`/`TppPaymentRecord`/`mapTppPaymentMethod` in `convex/tppParser.ts`) already existed; no manifest/regen (no new entity/guard/query — no #111 exposure), no schema change, no new Convex module.**

**Finding (verified first-hand against spec + code this turn):** the leads-commit increment (entry below) documented its next slice as "menus (a menu-catalog parser) or payments." Spec §6.2 lists payments as a single bullet ("Payments and reconciliation references from TPP, QuickBooks, and Nowsta") with the load-bearing detail in §6.4 ("Imported payment records retain source, external transaction ID, amount, date, type, event/client reference, and reconciliation state. Match by explicit provider/source IDs first…"). Verified before building: (1) the parser layer was already complete (`TppPaymentRecord` tppParser.ts:119, `ParsedCapsulePayment` :197, `parseTppPayment` :442, `parseTppPayments` :677, `mapTppPaymentMethod` :323) — only the `commitImportRun` materialization branch + UI were missing; payments threw the "not yet supported" catch-all. (2) The `Payment` entity (`src/sales/payment.manifest:39`) creates via `record` → `Payment_createViaRecord`, which REQUIRES `invoiceId` + `clientId` as non-nullable FKs with guards `self.invoice != null`/`self.client != null` (:101-102) + match constraints. (3) **There is no invoice import** (the 5 import datasets are events/contacts/venues/leads/payments; no invoice parser/`TppInvoiceRecord` exists), so a TPP `InvoiceID` can NEVER resolve to a Capsule invoice, and a TPP payment row carries no `ClientID`. A Payment entity therefore CANNOT be created from a TPP import without relaxing the invoice invariant — a domain-model change with broad blast radius (the `on PaymentSettled run Invoice.applyPayment` reaction, settlement, reporting all assume a real invoice). (4) This is consistent with §6.4: imported payments are **reconciliation references** matched (`Payment.markMatched`) against EXISTING Capsule payments, NOT new Payment entities. The reconciliation fields (`externalSource` incl. `tpp_legacy`, `externalPaymentId`, `providerTransactionIds`, `reconciliationStatus`) + commands (`markMatched`/`verifyReconciliation`/`disputeReconciliation`) already exist on Payment. (5) `CapsuleEntityType` includes `payment` (external-record-link.manifest:25) and `recordType` is a free string (:62), so a payment link needs NO schema/regen change.

**Fix (smallest spec-faithful diff — one new branch mirroring the proven loop + a dataset-aware UI):**
- `convex/importCommit.ts` — added a `payments` branch to `commitImportRun`. Per parsed TPP payment: idempotency skip via `findLink(recordType "payment")` (any active link = already staged/matched → skip, so re-runs are a true no-op); optional cross-dataset resolution of the external `EventID` → Capsule event (matching context); then a `pending_conflict` `ExternalRecordLink` (recordType `payment`, capsuleEntity `payment`, capsuleId `""`) staged in the reconcile queue with a resolution note describing the reconciliation context. **No Payment entity is created** — the link IS the committed artifact (the §6.4 reference model); `conflictStatus "pending_conflict"` is the correct "awaiting match" state, not a creation failure, so a staged link counts as `committed`. Revert is dataset-agnostic (already supersedes via `by_sourceImportRunId`), so payments revert for free. The unsupported-throw now names only `menus`.
- `src/features/admin/import/ImportRunDetailPage.tsx` — generalized the commit form to also accept `payments`: `supportedCommitDatasets` + `commitNoun` now cover payments; the JSON-textarea placeholder gained a payment-row example; the help text + actions guide explain the reconciliation-reference model (no Payment entity; matched later via `markMatched`); the error/notice strings updated.

**Key decision (documented):** imported TPP payments become reconciliation-reference LINKS, not Payment entities. Rationale: (a) `Payment.record` is hard-blocked on a Capsule invoice+client that no import produces; (b) §6.4 explicitly models imported payments as references to be matched (`markMatched`), retaining source/external-id/amount/date/type/event-client-reference/reconciliation-state — all of which a `payment` ExternalRecordLink + the Payment reconciliation fields capture; (c) it needs no schema/regen (no invariant change, no new entity). The resolution path is the existing reconcile workflow: operator matches a Capsule payment (`markMatched` sets `externalSource`/`externalPaymentId`) → `updateCapsuleId`/`resolveConflict` on the link (capsuleId = matched payment, `resolved`). Making `invoiceId` optional (to create real Payment entities for legacy payments) is a genuine domain decision with settlement/reaction blast radius — documented as a separate slice, not silently taken.

**Why it matters:** the #1 Critical-priority Import Datasets (Priority 2) moves from "venues + contacts + events + leads" to "venues + contacts + events + leads + payments" — the fifth and FINAL dataset in the spec's Slice-2 list ("contacts, deals, menu, venues, pack lists, and payment imports") to become reachable end-to-end. Imported TPP/QuickBooks/Nowsta payments become real reconciliation references in the queue (the §6.4 matching workflow operates on them), advancing §6.2/§6.4 and the TPP migration. Re-running is safe (per-record `findLink`/`upsertLink` dedup keyed on `(tenant, sourceSystem, recordType="payment", externalId)`).

**Verification:** `bun run check` GREEN (exit 0) — toolchain, ownership ledger, all manifest-slice contracts, typecheck 0, format clean, secrets, test:coverage, build ok (`ImportRunDetailPage` chunk 16.34 kB, up from 15.85), baseline-decay ok. Runtime write-path verified by inspection (the generator's `as any` casts hide runtime bugs — the recurring GREEN-but-broken trap): UI `commitImportRun({importRunId, rawRows})` (datasetType payments) → `loadCommitContext` → per payment: `findLink("payment")` (idempotency) → optional `findLink("event")` (context) → `upsertLink` (raw insert/patch, recordType "payment", capsuleEntity "payment", capsuleId "", conflictStatus pending_conflict) → `ImportRun_commit` (generated, committing→completed). The repo's recurring GREEN-but-broken failure modes do NOT apply (NO generated entity create → no `self.id`/`context.timestamp`/`_createVia*`-docId/FK-relation-key traps; `upsertLink` is an authored raw `ctx.db.insert/patch` so capsuleId "" is allowed — the events branch already uses capsuleId "" for pending_conflict; `payment` capsuleEntity is a valid enum literal; `payment.recordedAt` is a number guarded before `new Date(...).toISOString()`). No tests added (authored seam branch + UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; the change is an additive authored-seam branch mirroring the shipped venues/contacts/events/leads patterns using an already-shipped parser + a mechanical UI generalization, with no new guard/policy/approval beyond the existing `importAccess` role and no money mutation — no Payment entity is created or settled). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **No Payment entities are created** — every imported payment lands as a `pending_conflict` reconciliation-reference link awaiting a `markMatched` match; this is the spec §6.4 model, not an omission. (2) **Matching is a manual operator workflow** — the import stages references; an operator must match each to an existing Capsule payment (`markMatched`) then resolve the link (`updateCapsuleId`/`resolveConflict`). No auto-match heuristic is applied (§6.4: "Amount/date/name heuristics may suggest a match but must not silently finalize an ambiguous match"). (3) **Menus still NOT wired** — the only remaining dataset; needs a parser + a TPP source feed (Dish/Menu/MenuDish entities are standalone-creatable but no `TppMenuRecord`/TPP menu export exists to map). (4) A `main` push deploys Convex too (`vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'`, since cc24315), so the new authored action branch is live server-side once the Vercel build is READY.

---

**2026-07-26 — §6.2 Leads import WIRED (fourth dataset of the #1 Critical-priority Import Datasets; venues + contacts + events + leads now materialize):**

**Status: ✅ Shipped + `bun run check` GREEN (exit 0). Additive branch to the existing `convex/importCommit.ts` authored seam + a new `parseTppLead(s)` in `convex/tppParser.ts` + a dataset-aware UI generalization; no manifest/regen (no new entity/guard/query — no #111 exposure), no schema change, no new Convex module.**

**Finding (verified first-hand against spec + code this turn):** the events-commit increment (entry below) documented its next slice as "leads … same client-resolution path as events." Spec §6.2 "Open and historical pipeline/deals, stages, and close history" was unmet for leads: `commitImportRun` threw "not yet supported" for `datasetType === "leads"`. Verified before building: (1) the `TppLeadRecord` interface (`convex/tppParser.ts:83`) already declared the TPP opportunity row (LeadID/OpportunityName/ClientID/Stage/Probability/EstimatedValue/Source/ReferralSource/dates) and a `mapTppLeadStage` helper existed, but there was NO `parseTppLead`/`ParsedCapsuleLead`/`parseTppLeads` — only the materialization was missing. (2) The `Lead` entity (`src/sales/lead.manifest:51`) creates via the `capture` command → generated `Lead_createViaCapture` (`convex/mutations.ts:17004`, returns `{docId}`, salesAccess-guarded, PII-encrypting email/phone, `idempotencyKey`-deduped). (3) `clientId` is OPTIONAL on Lead (set only on conversion), so — unlike events — leads need NO cross-dataset client resolution: the Lead IS the pre-client inquiry. (4) `capture` hardcodes `stage: "new"` (no stage arg), the same shape as events landing at "planning"; the TPP stage is preserved on the link.

**Fix (smallest spec-faithful diff — new parser + one new branch mirroring the proven contact loop + a dataset-aware UI):**
- `convex/tppParser.ts` — added `ParsedCapsuleLead` (opportunityName/source/estimatedValue/stage/probability/external clientId/dates), a singular `parseTppLead` (TPP row → Capsule; `Source || "TPP Import"` so the capture non-empty-source constraint always holds; `parseTppMoney(EstimatedValue) ?? 0` so the ≥0 constraint holds; `mapTppLeadStage`), and a batch `parseTppLeads` (validates LeadID + non-empty OpportunityName required).
- `convex/importCommit.ts` — added a `leads` branch to `commitImportRun` (the venue/contact/event paths left byte-identical). Per parsed TPP lead: idempotency skip via `findLink(recordType "lead")`; then `Lead_createViaCapture({leadType:"company", source, estimatedValue, companyName: opportunityName, probability, notes, idempotencyKey})`. The TPP ClientID + mapped stage are folded into the Lead `notes` (the reconciliation signal) and the full raw row is preserved on the link's `rawSourceData`. An idempotent `ExternalRecordLink` (recordType `lead` → capsuleEntity `lead`; `resolved`/`pending_conflict`). Revert is dataset-agnostic (already supersedes via `by_sourceImportRunId`), so leads revert for free.
- `src/features/admin/import/ImportRunDetailPage.tsx` — generalized the venues/contacts/events commit form to also accept `leads`: `supportedCommitDatasets` + `commitNoun` now cover leads; the JSON-textarea placeholder gained a lead-row example; the error/help/Approve&Commit strings updated. The result counts (`committed/skipped/pending/parseErrors`) surface unchanged.

**Key decision (documented):** a TPP opportunity/pipeline row → a company-type Lead (`OpportunityName` → `companyName`), NOT a person Lead and NOT linked to a Client at import. The TPP row carries only `OpportunityName` (no first/last name), so a company Lead is the only spec-faithful mapping (spec §6.2: "does not invent substitutes for fields not present"). Linking the lead to a Capsule Client is the documented conversion workflow (`stageConversion` → `confirmConversion`, a guarded operator action), and the TPP `Stage`/`ClientID`/`CloseDate` are preserved on the link for §6.1 parallel-run reconciliation rather than applied at create (mirrors the events stage decision).

**Why it matters:** the #1 Critical-priority Import Datasets (Priority 2) moves from "venues + contacts + events" to "venues + contacts + events + leads" — the fourth dataset that materializes real entities end-to-end. Imported pipeline/deals become real Lead entities (the pipeline the sales workflow and LeadPipelinePage operate on). Re-running is safe (per-record `idempotencyKey` + `findLink`/`upsertLink` dedup keyed on `(tenant, sourceSystem, recordType="lead", externalId)`).

**Verification:** `bun run check` GREEN (exit 0) — toolchain, ownership ledger, all 9 manifest-slice contracts, typecheck 0, format clean, secrets, test:coverage, build ok (`ImportRunDetailPage` chunk 15.85 kB, up from 15.45), baseline-decay ok. Runtime write-path verified by inspection (the generator's `as any` casts hide runtime bugs — the recurring GREEN-but-broken trap): UI `commitImportRun({importRunId, rawRows})` (datasetType leads) → `loadCommitContext` → per lead: `findLink("lead")` (idempotency) → `Lead_createViaCapture` (generated, salesAccess-guarded, encrypts email/phone, `idempotencyKey` cached, returns `{docId}`) → `upsertLink` (recordType "lead", capsuleEntity "lead", conflictStatus resolved/pending) → `ImportRun_commit` (generated, committing→completed). The repo's recurring GREEN-but-broken failure modes do NOT apply (no `self.id`/`context.timestamp` in a create — Lead id comes from the generated command's `{docId}` return; `leadType:"company"` + non-empty `companyName` satisfies the capture identity constraint; `source` defaulted non-empty + `estimatedValue ≥ 0` satisfy the capture constraints; `_createVia*` returns `docId` read directly; no FK relation key at create — `clientId`/`clientContactId`/`proposalId` are null at capture; no datetime param). No tests added (authored seam + parser + UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; the change is an additive authored-seam branch mirroring the shipped venues/contacts/events patterns + a new parser following the existing `parseTppContact`/`mapTppLeadStage` shape + a mechanical UI generalization, with no new guard/policy/approval beyond the existing `importAccess`/`salesAccess` roles and no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **Imported leads always land at `stage: "new"`** regardless of TPP `Stage` — driving historical leads to their true Capsule stage (qualified/proposalSent/negotiating) via `updatePipeline`, and converting them to Clients via `stageConversion`/`confirmConversion`, is a separate slice. (2) **Leads are NOT linked to Clients at import** — a TPP lead's `ClientID` is preserved on the link, not resolved; conversion (a guarded operator action) is the documented next slice if historical lead→client linkage is wanted. (3) **A TPP opportunity → a company Lead** with `companyName = OpportunityName`; a person-type lead (given/family name) is not expressible from the TPP row shape. (4) **Menus/payments still not wired** — menus have no parser; payments reference external invoice/event ids needing cross-dataset resolution. The next slice is menus (a menu-catalog parser) or payments. (5) ~~needs a human `npx convex deploy -y`~~ CORRECTED 2026-07-26: a `main` push deploys Convex too — `vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'` (since cc24315, 2026-07-24), so pushed authored actions are live server-side once the Vercel build is READY; AGENTS.md §Deploying updated to match.

---

**2026-07-26 — §6.2 Events import WIRED (third dataset of the #1 Critical-priority Import Datasets; venues + contacts + events now materialize):**
...
**Finding (verified first-hand against spec + code this turn):** the contacts-commit increment (entry below) documented its next slice as "events with on-the-fly client/venue resolution" — the cross-dataset blocker is now cleared because venues + contacts already materialize as `ExternalRecordLink`s. Spec §6.2 ("Existing 2,103 Events") + §5.3 ("An imported TPP Event uses the same create-proposal command as a native Capsule Event") + §6.1 ("conflicts enter a review queue") were unmet for events: `commitImportRun` threw "not yet supported" for `datasetType === "events"`. Verified before building: (1) `parseTppEvents`/`ParsedCapsuleEvent` (`convex/tppParser.ts:442`) already parses EventID/EventName/ClientID/VenueID/dates/headcount/status — only the materialization was missing. (2) The Event entity (`src/operations/event.manifest:542`) has NO literal create command; `planEngagement` is promoted to the create verb → generated `Event_createViaPlanEngagement` (`convex/mutations.ts:7862`), returns `{docId}`, hardcodes `stage: "planning"` + `plannedAt = now()` (no `stage` arg → the TPP-mapped stage cannot be injected at create). (3) `clientId` is `required` (no default) and is the EXTERNAL TPP `ClientID` in the parsed row → must be resolved to a Capsule id via the prior contacts import before create.

**Fix (smallest spec-faithful diff — one new branch mirroring the proven venue/contact loop + a dataset-aware UI):**
- `convex/importCommit.ts` — added an `events` branch to `commitImportRun` (the venue/contact paths left byte-identical). Per parsed TPP event: idempotency skip via `findLink(recordType "event")`; **cross-dataset resolution** of the external `ClientID` → Capsule client via `findLink(recordType "contact")` (the prior contacts import wrote these) and the external `VenueID` → Capsule venue via `findLink(recordType "venue")`; then `Event_createViaPlanEngagement` with the resolved ids. An event whose client was NOT imported (e.g. a TPP company id never imported as a contact) becomes a `pending_conflict` link (reconcile queue) — it does NOT fabricate a client. Then an idempotent `ExternalRecordLink` (recordType `event` → capsuleEntity `event_record`; `resolved` on success, `pending_conflict` on per-record failure). Revert is dataset-agnostic (already supersedes via `by_sourceImportRunId`), so events revert for free.
- **Required-field synthesis (documented in-code with `ponytail:` comments):** `Event_createViaPlanEngagement` requires several non-empty fields the TPP row does not carry directly — `eventType` (synthesized from the TPP EventType slug, else "Imported Event"), `primaryContactName` (derived from the imported contact's name stored on its link, else "Imported Contact"), `expectedHeadcount` (guarded ≥ 1), and `startsAt`/`endsAt` (`endsAt > startsAt`, defaulting a 1-hour window when EndTime is absent; an event with no start date → `pending_conflict`). Slug `occasionId`/`serviceStyleId` are deliberately OMITTED (they are not real Capsule ids and would fail the `ref` FK existence check).
- `src/features/admin/import/ImportRunDetailPage.tsx` — generalized the venues/contacts commit form to also accept `events`: `supportedCommitDatasets` + `commitNoun` now cover events; the JSON-textarea placeholder gained an event-row example; the error/help/revert-confirm strings updated. The result counts (`committed/skipped/pending/parseErrors`) surface unchanged.

**Key decision (documented):** the TPP-mapped stage is NOT applied. `Event_createViaPlanEngagement` hardcodes `stage: "planning"` and offers no `stage` argument, and reaching `sales_lock`/`completed` from a fresh import requires driving the full guarded transition chain (`submitForApproval`→`approve`→`lockForSales`→…), several of which carry `eventManageAccess`/`salesManageAccess` role guards — wrong for a bulk historical import (re-approving 2,103 past events). Imported events therefore land at `planning`; the raw TPP `EventStatus` is preserved on the link's `rawSourceData` for parallel-run reconciliation (§6.1 "Legacy source fields are visible for reconciliation"). Status reconciliation (driving imported events to their true Capsule stage) is the documented next slice.

**Why it matters:** the #1 Critical-priority Import Datasets (Priority 2) moves from "venues + contacts" to "venues + contacts + events" — the third dataset that materializes real entities end-to-end, and the most important one (2,103 TPP events; events are the spine proposals/payments/staffing reference). Imported events become real Event entities — the entity §5.3's create-proposal-from-event and §6.5's parallel-run comparison act on. Re-running is safe (per-record `idempotencyKey` + `findLink`/`upsertLink` dedup keyed on `(tenant, sourceSystem, recordType="event", externalId)`).

**Verification:** `bun run check` GREEN (exit 0) — toolchain, ownership ledger, all 9 manifest-slice contracts, typecheck 0, format clean, secrets, test:coverage, build ok (`ImportRunDetailPage` chunk 15.45 kB), baseline-decay ok. Runtime write-path verified by inspection (the generator's `as any` casts hide runtime bugs — the recurring GREEN-but-broken trap): UI `commitImportRun({importRunId, rawRows})` (datasetType events) → `loadCommitContext` (auth via internal query) → per event: `findLink("event")` (idempotency) → `findLink("contact")` resolve client (pending_conflict if absent) → `findLink("venue")` resolve venue (optional) → `Event_createViaPlanEngagement` (generated, eventManageAccess/salesAccess-guarded, `idempotencyKey` cached, returns `{docId}`) → `upsertLink` (recordType "event", capsuleEntity "event_record", conflictStatus resolved/pending) → `ImportRun_commit` (generated, committing→completed). The repo's recurring GREEN-but-broken failure modes do NOT apply (no `self.id`/`context.timestamp` in a create — Event id comes from the generated command's `{docId}` return; resolved clientId/venueId are real same-tenant ids so the `belongsTo client`/`ref venue` FK checks pass; slug occasionId/serviceStyleId omitted so no bogus-FK throw; `_createVia*` returns `docId` read directly; no relation-key reuse; datetime params are real numbers). No tests added (authored seam + UI; AGENTS.md: do not add tests unless the owner asks).

**Cross-model review:** not run this increment (autonomous-loop cadence; the change is an additive authored-seam branch mirroring the shipped venues/contacts patterns + a mechanical UI generalization, with no new guard/policy/approval beyond the existing `importAccess`/`eventManageAccess`/`salesAccess` roles and no money mutation). The merge gate's independent cross-model review still applies at PR time.

**Honest scope notes (documented, NOT this increment):** (1) **Imported events always land at `stage: "planning"`** regardless of TPP `EventStatus` — driving historical events to their true Capsule stage (sales_lock/completed/cancelled) via the guarded transition chain is a separate slice. (2) **Client resolution requires the contact to have been imported first** — a TPP event referencing a company `ClientID` (not a contact) yields `pending_conflict`; the company→Client resolution remains the documented deferred slice. (3) **`eventType`/`primaryContactName` are synthesized** when the TPP row lacks them (placeholder free text); the full raw row is preserved on the link's `rawSourceData`. (4) **Leads/menus/payments still not wired** — leads reference external client ids, payments reference invoice/event ids, menus have no parser yet. The next slice is leads (same client-resolution path as events) or status reconciliation for imported events. (5) ~~needs a human `npx convex deploy -y`~~ CORRECTED 2026-07-26: a `main` push deploys Convex too — `vercel.json` `buildCommand` is `convex deploy --cmd 'vite build'` (since cc24315, 2026-07-24), so pushed authored actions are live server-side once the Vercel build is READY; AGENTS.md §Deploying updated to match.
...
- **P1 round 1 ("render from frozen revision, not live-recomputed") → round 2 ("do not trust caller-supplied stored amounts") — RESOLVED by recomputing.** Round 1 suggested carrying the stored `amount` for reproducibility; an intermediate revision did that. Round 2 then correctly flagged that carrying the stored amount TRUSTS a value the command-API path can write without the authoritative recompute seam (`convex/http.ts` → generated `ProposalLineItem_*` accept any non-negative `amount` — the same §5.4 "Finding B" generator-level bypass), which could render an incorrect breakdown disagreeing with the operator PDF. **Final resolution: do NOT project or trust the stored amount — recompute it in the PDF from the frozen inputs via the central calc, exactly as the operator PDF already does.** This is provably correct for accepted proposals: their line inputs are IMMUTABLE (all three line commands `addLine`/`reviseLine`/`removeLine` guard `self.proposal.status == "draft"`), so recomputing reproduces the accepted terms, and it keeps the client + operator PDFs byte-identical (single source of truth). No `proposalLineItems` amount is projected (the intermediate `amount?` field was reverted); the shared PDF path is untouched.
- **Round 3 — 1 residual DOCUMENTED (not fixed), 1 out-of-scope:** (a) codex flagged that the breakdown is recomputed while the Estimate summary still reads the STORED `proposal.subtotal/total`, so for command-API-created proposals whose stored line inputs disagree with the stored totals (the §5.4 Finding B generator gap — `do-not-edit` `http.ts` dispatch), the client PDF could show a breakdown that doesn't foot to the Estimate. This does NOT affect the UI operator surface (the `*AndRecompute` seam keeps stored == recomputed, so breakdown == Estimate there) — only the command-API edge case, whose root cause is the unfixed generator gap. The clean in-repo mitigation (render the Estimate from the recomputed `priced` totals when pricing lines exist, so the PDF is always internally consistent) is **documented as a follow-up**, not taken this increment: it touches the shared `generateProposalPdf` Estimate path (operator-visible), the code is already pushed, and a concurrent autonomous loop is active on this checkout (see Push status) — so a 4th shared-path commit amid that churn is disproportionate for a Finding-B edge case. (b) Codex also flagged `.claude/review-gate-ignore` excluding `package.json` from the review gate — that is the concurrent loop's own `b4e1aa3` chore commit, NOT this increment; left for that work's owner.

**Push status: PUSHED (by a concurrent autonomous loop, not by this session).** A parallel loop committed `b4e1aa3` ("[chore] Add review-gate generated-path exclusions") directly on top of this session's `eef6990` and pushed `origin/main` → `b4e1aa3` (verified via `git ls-remote`: remote `refs/heads/main` == `b4e1aa3`, and `eef6990` is an ancestor). So all three §4.2 commits (`0dadd28` feat, `7911525` fix, `eef6990` recompute) are on `origin/main` and `bun run check` is GREEN. ⚠ The new authored `clientPortal.ts` query change still needs a **human-authorized `npx convex deploy -y`** to take effect server-side (AGENTS.md: Convex deploys are human-only; a frontend `main` push does not deploy Convex). Until then the portal PDF still renders the flat Estimate.
...
|-------|--------|----------|--------------|-----------------|---------------|
| **Slice 0** | ✅ Strong | 0 | 90% | Event detail ✅, PackList separation ✅, ServiceStyle ✅, Occasion ✅, ReferralSource ✅, Salesperson/Owner ✅, Sales Lock ✅ | Event creation fields complete (occasion/service-style/salesperson/referral all wired on the create form 2026-07-26) |
...
| **Slice 4** | ✅ Complete | 0 | 100% | Kitchen ✅, inventory ✅, staffing ✅, equipment ✅, performance event linkage ✅, scorecards ✅, one-on-ones ✅, hiring pipeline ✅ | — (all spec §9 items done; Slice 4 spec-complete 2026-07-26) |
...
| Event | ✅ DONE | events table with full lifecycle incl. sales_lock/executing/final | Added ServiceStyle, Occasion, and ReferralSource relations (referralSourceId on Event 2026-07-26); salesperson/owner via assignedToId, UI-wired on the create form 2026-07-26 |
...
| Salesperson/Owner | ✅ DONE | assignedToId on events; people.role enum includes sales_staff, sales_manager, owner | No explicit salesperson entity - uses people with role; create-form selector wired 2026-07-26 (sales-role filter) |
| Referral Source | ✅ DONE | src/sales/referral-source.manifest with common sources (Website, Referral, Phone, etc.), active/inactive state, display order | Leads + Events reference ReferralSource via optional referralSourceId relation (Event.referralSourceId + create-form selector added 2026-07-26); Lead keeps its existing free-text source field for flexibility |
...
| Candidate/Application | ✅ DONE | `src/workforce/hiring.manifest`; `candidates` table in schema.ts; commands apply/advance/reject/hire; stage enum application→hired/rejected; `sourceSystem`+`externalCandidateId`+`rawSourceData` provenance; `CandidatesPage` + `/staff/hiring` | NEWLY IMPLEMENTED 2026-07-26 (closes §9.3; KM-JSON ingest idempotent via `hiringPipeline.ingestKmCandidates`) |
| Interview | ✅ DONE | `src/workforce/hiring.manifest`; `interviews` table in schema.ts (`by_candidateId` index); commands schedule/recordOutcome; outcome pending/passed/failed; `sourceSystem`+`externalInterviewId`+`rawSourceData` | NEWLY IMPLEMENTED 2026-07-26 (child of Candidate; KM outcome recorded on ingest) |
...
| Sync Error | ✅ DONE | `syncErrors` table + `SyncError` entity (`src/sales/sync-error.manifest`); `ingestInboundMessage` records failures idempotently; `SyncErrorsPanel` on the inbox | Provider-neutral retryable queue; Retry is client-side (re-runs ingest) + Dismiss |
...
### ✅ 3.4 Event Creation Fields — DONE

**Done:**
- Date/guests/venue present in Event entity
- EventCreatePage.tsx with full form
- Fields: title, eventType, expectedHeadcount, startsAt/endsAt, primary contact (name/email/phone), accessibility needs, service requirements, operational requirements, budget amount, quoted price, client + venue selection
- Occasion enum ✅ + Service-style enum ✅ (prior increments)
- Labeled salesperson selector ✅ (Event.assignedToId → Person, sales-role filter) — wired 2026-07-26
- Referral source selector ✅ (Event.referralSourceId → ReferralSource) — added + wired 2026-07-26

**Closed (2026-07-26):** occasion, service-style, salesperson, and referral source are all captured on the Event create form. `eventType` free-text is retained for raw TPP parity (occasion is the canonical enum).

**Remaining (NOT this increment):** extend the `EventDetailPage` edit path with salesperson/referral selectors (matches the create-only scope of the prior reference-field increments).
...
### 🟡 6.1 Import Framework — PARTIAL (entities + venues + contacts commit/revert REAL 2026-07-26; events/leads/payments/menus now WIRED 2026-07-26 (all six §6.2 datasets materialize))
...
**2026-07-26 update (see Changes This Update):** the commit/revert write path is now REAL for the **venues** and **contacts** datasets via the authored `convex/importCommit.ts` seam (`commitImportRun`/`revertImportRun` actions + UI wiring). The generated `ImportRun_commit`/`ImportRun_revert` (status-only) are wrapped by seam actions that actually materialize Venue/Client entities + idempotent ExternalRecordLinks and (revert) supersede them. A TPP contact → a person-type `Client` (NOT a `ClientContact`, which needs a parent client = cross-dataset). This corrects the prior FALSE "Slice 2 100% complete" — the framework previously wrote zero business data. Events/leads/payments/menus materialization is now WIRED too (see the Changes This Update entries) — all six §6.2 datasets materialize end-to-end: events resolve cross-dataset externalId→Capsule-id, leads/menus need no resolution, payments stage as §6.4 reconciliation-reference links, and menus → Dish catalog entities.
...
### ✅ 6.2 Required Datasets — DONE (all six datasets materialize 2026-07-26: venues + contacts + events + leads + payments + menus)
...
- Menu catalog, categories, prices — ✅ WIRED 2026-07-26: `parseTppMenus` → Dish entities via `importCommit.ts` (the sixth/final dataset; TPP `work/dishes.csv` feed). Price columns empty in the real feed, preserved on the link; Menu/MenuDish pricing graph is a follow-up if a feed with real prices arrives
...
### ✅ 6.3 Browser-Extracted Pack Lists — DONE

**Status:** ✅ DONE 2026-07-26 — browser-extracted per-event pack-list JSON now flows through the same ImportRun/ExternalRecordLink framework and materializes a real `PackList` + `PackListItem` rows via the new `datasetType === "pack_list"` branch in `convex/importCommit.ts` (`PackList_createViaOpen` + `PackListItem_createViaAddItem`). Source event is resolved from the prior events import (`findLink recordType:"event"`; pending_conflict if not imported); idempotent per-event (`externalId = sourceEventId`, re-run skips an already-linked event); unrecognized items land as description-only free-text lines (dishId null) — the §6.3 "unrecognized items may remain as imported free-text lines" path. Parser: `parseTppPackList(s)` + `mapPackListUnit` in `convex/tppParser.ts`; `pack_list` added to `ImportDatasetType` + `CapsuleEntityType` (additive enum members, regen-propagated). TPP has no bulk export, so the operator pastes the extracted JSON; the browser-scraper producer is out of scope (Capsule-side consumer was the missing half).
...
**Shipped (2026-07-26):**
- `src/workforce/hiring.manifest` — `Candidate` (apply/advance/reject/hire; stage enum) + `Interview` (schedule/recordOutcome; outcome enum) child entity. Each carries `sourceSystem` + `external*Id` + `rawSourceData` provenance.
- `convex/kmParser.ts` + `convex/hiringPipeline.ts` (`ingestKmCandidates`) — idempotent find-or-upsert by source id (re-import updates without duplication; the §9.3 core requirement).
- `CandidatesPage` + `/staff/hiring` route + nav.

See the top changelog entry for the full fix + honest scope notes.
...
**Gaps:**
- NO connected inbox
- ✅ Threading entities BUILT — `MessageThread` (`src/sales/message-thread.manifest`) + `Message` (`src/sales/message.manifest`) + `messageThreads`/`messages` tables; `ingestInboundMessage` idempotent action + one-click `qualifyThreadAsLead`; `MessageInboxPage` at `/clients/inbox`
- ✅ Provider message IDs BUILT — threads + messages carry provider thread/message IDs, sender identity, provider timestamps, raw payload
- NO reply linkage
- NO bounce/failed state tracking
- NO webhook ingestion for inbound email

**Evidence:**
- ✅ `MessageThread` + `Message` entities ARE in schema (`messageThreads` + `messages` tables); `MessageInboxPage` inbox UI at `/clients/inbox`; `ingestInboundMessage` idempotent ingest action; §4.4 retryable `SyncError` queue
- NO provider inbox integration (OAuth Connection + inbound webhook/poller) — the delivery layer that would call `ingestInboundMessage` is not built; threads populated via operator "Log inbound" only
...
|----------|------|--------|--------|--------------|-----------|--------|
| ~~1~~ | **Import Framework** | Large | Critical | None | Foundation for entire TPP migration - blocks Slice 2 | 🟡 PARTIAL (2026-07-26): entities + reconcile UI + dashboard + cutover + **venues + contacts + events + leads commit/revert REAL** (`convex/importCommit.ts`); payments + menus now WIRED 2026-07-26 (all six §6.2 datasets materialize). Prior "✅ DONE" overstated. |
| 2 | **Import Datasets** | Medium | Critical | None | Events/Contacts/Leads/Menu/Venues/Payments import - 2,103 TPP events | 🟡 PARTIAL (2026-07-26): parsers exist for events/contacts/leads/venues/payments (`convex/tppParser.ts`); **VENUES + CONTACTS + EVENTS + LEADS** materialize end-to-end via `importCommit.ts` (contacts → person-type `Client`; events → `Event` with cross-dataset client/venue resolution; leads → company-type `Lead` pre-client inquiry). menus + payments now WIRED 2026-07-26 (venues + contacts + events + leads + payments + menus ALL materialize — all six datasets). Prior "✅ DONE 6 datasets" was FALSE. |
...
| 17 | **Venue Profile (Full Depth)** | Large | High | None | Venue management and logistics - blocks venue selection | 🟡 PARTIAL (2026-07-26): room/space sub-entity now DONE (`VenueRoom` entity + `VenueRoomsPanel` UI in `VenueDetailPage`) — closes the concrete §8.1 "room/space details" gap; only "scorecard metrics" (overlaps shipped Company Scorecard dashboard) remains. Note: the §8.1-§8.2 detailed rows below are STALE (all 12 logistics fields + UI are wired). §8.2 venue-logistics snapshot now frozen into the proposal revision (2026-07-26) — shared/accepted proposals reproduce venue logistics after later venue edits (§5.2/§5.5). |
...
| 26 | **Hiring Pipeline** | Large | Medium | None | HR operations - tracks candidates through stages | ✅ DONE 2026-07-26 — `src/workforce/hiring.manifest` (Candidate + Interview) + idempotent KM-JSON ingest (`convex/hiringPipeline.ts`) + `CandidatesPage` + `/staff/hiring` route + nav. Closes the last Slice-4 HR gap. |
| 27 | **One-on-Ones** | Medium | Medium | Role Scorecards | Staff development - structured manager meetings | ✅ DONE — shipped `v0.0.34`/`e5403dc` (OneOnOne + OneOnOneAction entities + OneOnOnesPage + `/staff/one-on-ones` route). The prior ❌ row here was stale (corrected in passing 2026-07-26). |
...
| Priority | Item | Effort | Impact | Dependencies | Why |
|----------|------|--------|--------|--------------|-----|
| 31 | **Browser-Extracted Pack Lists** | Large | High | Import Framework, Pack Templates | Data migration - extracts TPP pack lists from browser. ✅ DONE 2026-07-26 (browser-extracted pack list JSON → PackList + PackListItem via importCommit.ts pack_list branch) |

### Provider (Slice 5 — Integration completion)

| Priority | Item | Effort | Impact | Dependencies | Why |
|----------|------|--------|--------|--------------|-----|
| 32 | **Email Inbox/Threading** | Medium-Large | High | Integration Contract | Connected inbox; reply tracking; conversation history. 🟡 PARTIAL (2026-07-26): threaded data model (messages + messageThreads), idempotent ingest action, one-click qualify, staff inbox UI at /clients/inbox, AND §4.4 retryable sync-error queue all DONE. Remaining: provider OAuth Integration Connection per email/SMS/social + inbound delivery pipeline (webhook/poller) + outbound delivery worker — need provider credentials. |
...
**Last updated:** 2026-07-26 (§9.2 Role Scorecards NEWLY IMPLEMENTED; corrected false "DONE" claims — §9.3 Hiring Pipeline and §9.5 One-on-Ones are genuinely NOT BUILT; §4.4 retryable sync-error queue DONE (SyncError entity + ingest-failure recording + SyncErrorsPanel))
...
3. ~~Priority 27: One-on-Ones~~ ✅ DONE — shipped v0.0.34 (OneOnOne + OneOnOneAction + OneOnOnesPage + /staff/one-on-ones).
4. ~~Priority 26: Hiring Pipeline~~ ✅ DONE — Candidate + Interview entities, idempotent KM ingest, CandidatesPage, /staff/hiring route (2026-07-26). Slice 4 is now spec-complete.
