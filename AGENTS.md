...
- Generated files are still never hand-edited: manifest changes go through
  ...
  commit (`.builder/` and `scripts/manifest-builder/` are committed here).

# Documentation (read this whenever you are making any changes)

RYAN_APPROVED 7-23-2026 Created by Ryan
docs/reference/manifest-llms-full.txt
...
bun run check            # toolchain + ownership + proof:emit + check:proof + manifest-registry + manifest-breaking (vs last [release], acks in scripts/manifest-breaking-acks.json) + manifest-fmt (--check) + manifest-scan (--strict) + manifest-integration (all domain guards, generated/proof/guard.*.json) + design-vocab + typecheck + format:check + secrets + test:coverage + build + baseline:decay
...
no sibling checkout, `BUILDER_DIR`, or cross-repository pin sync. Commit tooling
fixes in Capsule and regenerate owned output in the same change. See
...
1. Backend first, if `convex/` or manifests changed:
   ...
   instance the ONLY path is `scripts/deploy-backend.sh --expect <main sha>` on
   the Linux box (`scripts/deploy-production.sh` runs it there over SSH); it runs
   this same command after it verifies the commit.
