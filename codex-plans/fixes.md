...
## 2026-09-21: PL-BOOKING retry-link repair (issue #391)
- RED regression first: the new test reproduced the defect on then-current source — a retry that reused the checkpointed proposal completed the QuoteSubmission while the reused draft proposal stayed unlinked, so `createEventFromAcceptedProposal` would book a second Event (`red.log`: 6 old cases green, 1 new red).
- Fixed: `src/sales/proposal.manifest` `stageEventLink`/`linkEvent` now guard `draft or accepted` (regenerated runtime), a draft link emits a null `dishSelectionProposalId` so unaccepted menu never cascades; `convex/quoteBuilder.ts` `processQuoteSubmission` recovers a lost event checkpoint from the saved proposal's live event before any create and reconciles the canonical link before completion; new internal `convex/lib/proposalEventCreation.ts` `linkConvertedQuoteProposal` reads through generated `getProposal`/`getEvent`, validates the Event before any success (including the same-event no-op), refuses client mismatch and differing existing links, and links only via the staged handshake — a throw rolls the staged pointer back.
- One correction round: the soft-deleted recovered Event was first reported as completed; the Event is now validated on every path and the deleted-Event case fails cleanly with checkpoint and pointer preserved (`207-validate-recovered-event.log`).
- Proof: 4 new runtime cases (seeded durable checkpoint state + real retry/actions/commands/readback), focused 5 files/39 (`focused-final.log`), full `bun run test` 197 files/918 (`test.log`), full `bun run check` exit 0 (`check-final.log`), review VERDICT: APPROVE gpt-5.6-sol (`review.log`). AC-412/AC-433 → PASS; receipt codex-plans/whole-spec-audit-2026-09-20/quote-retry-evidence-2026-09-21.md. No commit/push yet; post-commit regeneration gate and CI follow separately.

## 2026-09-22: PL-BOOKING booking-UI repair (AC-411, issue #392)
