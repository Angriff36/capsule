...
**Resolved source discrepancy at row 242 (2026-09-09):** The nested row 244 is wrong in both identity and unit. Use Gorgonzola Cheese Crumbs, 5.21875 pounds for 167 servings (0.03125 pound per serving). Exact prep row 242 gives 83.5 dry ounces; Event_Menu_Item_Production C32/E32 and the visually rechecked menu-with-prep.pdf page 1 both show the rounded 5.22 pounds. Menu and packing sources independently identify gorgonzola separately from fennel. Preserve the original erroneous leaf above as evidence, not a second ingredient demand. Packing's 5.177-pound requirement is a separate source representation, not the formula authority for the prep recipe.

`recipes/mid-summer-salad-source.json` records four qualified direct ingredients, the complete dressing component reference, service method, correction evidence and remaining walnut inputs. Actual generated-runtime qualification in `.artifacts/operations-source-study/reproduce-salad-source.ts` verifies those four direct demands and all eight dressing ingredients at 167/334 servings, with separate gorgonzola and fennel. The complete salad repair is not applied: the walnut candying method and finished yield remain unconfirmed; owner method clarification is pending. Do not invent a finished yield from the walnut work quantity.
