...
## Reviewed menu associations (2026-09-09)

The 191 unique source associations include 141 rows whose parent exactly matches one of Ashley's 15 menu-line dish names. The other 50 unique parents describe non-menu equipment/rental/event contexts. Together with 27 ambiguous and 16 unmatched rows, those 93 rows require their appropriate source relationship, not an invented dish link. All 141 selected rows currently have no dish/event-dish/container relationship in the saved snapshot.

The explicit snapshot-bound selection and PDF page/bounding-box evidence are tracked in `recipes/ashley-packing-associations.json`. Its mutation args target `lib/culinaryOperations:reconcileImportedPackingAssociations`; operation key ends with `9218f2b9e5b801378f001464fd6e6cc7d42765a4ae4292c4f4fbd6a266cbf0a2`. Refresh source review/versions against live state before applying. This file is a reviewed snapshot plan, not proof of applied records.

The batch seam calls Manifest-owned `PackListItem.restoreImportedAssociation` and stores a payload-bound materialization receipt containing every original row and source reference. The generated command sets only dishId, eventDishId, internal associationSource and ordinary revision metadata. It preserves description, quantity/unit, packed work, status, and timestamps. It checks reviewed versions/description, same active event-menu line, and pre-existing relationships/provenance. Removed menu lines and completed/dispatched history cannot receive this repair. Exact replay returns its receipt without writes, including after later staff edits; a conflicting payload with the same operation key is rejected.

Initial direct database mutation failed the existing Manifest integration guards and was replaced with this generated-command path; those guards were not weakened. Builder regeneration and proof emission produced the owned output. Actual runtime qualification covers atomic rollback when a later item is stale, duplicate/foreign/history/removed-menu rejection, direct command description/provenance checks, full original-field preservation and conflicting replay. A separate source-derived isolated runtime fixture exercised all 141 selected Ashley rows against 15 menu lines without quantity changes, duplicates, or replay writes. These checks use remapped isolated IDs, not production records. Actual live application, human-readable association presentation and connected scaling/model repairs remain required.

## Applying the reviewed association plan (2026-09-09)

`scripts/repair-tpp-packing-associations.ts` previews and executes the existing atomic association repair. It does not infer new links or refresh expected versions automatically. The source's snapshotSha256 must match the supplied snapshot bytes, and every explicit item/menu/list/event version and identity must agree with that reviewed snapshot.

```powershell
bun scripts/repair-tpp-packing-associations.ts --source codex-plans/source-backed-operations/recipes/ashley-packing-associations.json --snapshot .artifacts/operations-source-study/live-packing-20260909.json --out .artifacts/tpp-packing-association-repair
```

Preview writes plan.json and plan.sha256 with original packing rows, expected associations and exact mutation args. The saved Ashley snapshot produces141 selected rows and hash ebd0cc40957a566540bc3a036b2ffb0f05a9dfb5c965ee3152ef81dc1b85e307. This is a saved-snapshot qualification, not a current production preview. Before first live application, compare a fresh snapshot to source evidence and explicitly review any changed identity, menu selection, version or physical work. A materially revised plan needs a new operation key; keep the original plan and inputs for recovering an uncertain previous attempt.

After the authorized release makes the seam available, append `--apply --expected-plan-sha256 <reviewed-hash> --url <explicit-Convex-URL> --tenant <tenant-ID>`. Credentials use the existing agent session. The command writes a separate attempt receipt under `<out>/<plan-hash>/` before invoking the mutation, saves acknowledgement before readback, and captures all selected current records. First application verifies restored links and preserved packing fields. Exact retries use the server's payload-bound transaction receipt; current differences are reported without overwriting subsequent staff work or another repair. A recovered transaction is explicitly distinguished from an unchanged-current-state verification.

Association repair and fluid-unit correction touch five of the same rows. Apply the reviewed associations first, capture a fresh packing snapshot, then generate the fluid-unit preview from that snapshot so its expected item versions include the association repair. Do not apply both old previews blindly. Actual isolated CLI qualification exercised141 associations, then all five source-confirmed corrections with existing packed quantities. Required and packed volume convert together, associations/provenance/status/timestamps survive, and both exact retries are no-ops. Retrying the original association plan after fluid correction reports five current differences while preserving those corrected records.

The local HTTP qualification adapter invoked actual Convex test mutations/queries; it was not authenticated production or Clerk integration proof. A failed readback retained the transaction acknowledgement in its own attempt file; retry recovered without writes. Nine offline malformed/stale/foreign/history/source cases also rejected before authentication. No production data has been changed.
