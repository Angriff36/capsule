...
### Completed prep: recorded output is missing from remaining-work reconciliation (#335)

Current authoritative reproduction at a19e35da: .artifacts/operations-source-study/qualify-completed-prep-resize.ts uses actual generated commands to create the source asparagus rate0.1875lb/serving, create167servings/31.3125lb prep, assign/start/complete it with30lb actual output, and then resize through200,240,120,0,200,200. Ingredient demand becomes37.5,45,22.5,0,37.5,37.5lb. Required additional work is7.5,15,0,0,7.5,7.5lb, but open prep remains0 in every case. The original planned31.3125lb, actual30lb, assignee and completion timestamp remain; its version still increments on every resize. Saved before/after: completed-prep-resize.json.

The actual client synchronizer also creates/refreshes nothing and returns taskCount1. Its EventPrepTask input type lacks completedQuantity, and its template map chooses a single row; adding a server-created supplemental row alone would therefore leave client coordination unable to account for cumulative completed work or reliably select the open row. This evidence changes the implementation boundary: both server reconciliation and the shared client input/matching path must understand the work balance. A fix confined to syncServings or a display label is insufficient.

Issue https://github.com/Angriff36/capsule/issues/335 records the distinct remaining-work defect, related to311/310. Next repair must retain immutable completed work, use actual completedQuantity rather than original planned quantity, reconcile an identifiable open balance for each exact event-menu line/recipe step, convert compatible work units, and preserve staff edits. Verify partial and subsequent completions, repeat edits, decrease/86/restore, multiple completed rows, recipe/dependency changes and absence of ingredient double counting. No source quantities, durations or extra ingredients are invented by this arithmetic. No code fix or live repair has yet been applied for this gap.
