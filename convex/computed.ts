...
// Computed property helpers (15 entit(y/ies)).
...
/** Preload nested relations for Invoice computeds (mutates doc in place). */
export async function hydrateComputedRelationsForInvoice(ctx: any, doc: Record<string, any>): Promise<void> {
  const docId = doc._id;
  (doc as any).payments = await ctx.db.query("payments").withIndex("by_invoiceId", (q: any) => q.eq("invoiceId", docId)).collect();
}
...
export function computeInvoice(doc: Record<string, any>): Record<string, any> {
  return {
    totalPaid: ((doc.payments) ?? []).map((p: Doc<"payments">) => ((((p.status === "completed") && (p.deletedAt == null)) ? p.amount : 0))).reduce((acc: number, v: unknown) => acc + (typeof v === "number" ? v : 0), 0),
    settledPaymentCount: ((doc.payments) ?? []).filter((p: Doc<"payments">) => (((p.status === "completed") && (p.deletedAt == null)))).length,
  };
}
