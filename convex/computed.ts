...
/** Preload nested relations for Vendor computeds (mutates doc in place). */
export async function hydrateComputedRelationsForVendor(ctx: any, doc: Record<string, any>): Promise<void> {
  const docId = doc._id;
  (doc as any).orders = await ctx.db.query("vendorOrders").withIndex("by_vendorId", (q: any) => q.eq("vendorId", docId)).collect();
}
