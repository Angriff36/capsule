...
user_invocable: true
allowed-tools: Read, Grep, Glob, Edit, Write, Bash(bun run typecheck:*), Bash(bun run test:*), Bash(git status:*), Bash(git diff:*), Bash(npx @cobusgreyling/loop-worktree:*)
---
...
- Worktree isolation is mandatory in this repo: edit only inside your
  attempt's `.loop-worktrees/<run-id>` worktree, never the main checkout
  (see loop-constraints.md).
