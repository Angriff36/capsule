...
user_invocable: true
allowed-tools: Read, Edit, Write, Bash(npx @cobusgreyling/loop-context:*), Bash(npx @cobusgreyling/loop-cost:*), Bash(npx @cobusgreyling/loop-worktree:*)
---
...
- `loop-budget` — the per-run `--token-budget` here comes from loop-cost's
  realistic estimate; loop-budget.md still governs the _daily_ cap across runs.
