...
// Computed property helpers (35 entit(y/ies)).
...
export async function hydrateComputedRelationsForEvent(ctx: any, doc: Record<string, any>): Promise<void> {
  const docId = doc._id;
  {
    const __fk = ((doc as any) as any).serviceStyleId;
    ((doc as any) as any).serviceStyle = __fk != null ? await ctx.db.get(__fk as any) : null;
  }
  {
...
export function computeEvent(doc: Record<string, any>, { user }: { user: any }): Record<string, any> {
  const __isTerminal = ((doc.stage === "cancelled") || (doc.stage === "closed_out"));
  doc.isTerminal = __isTerminal;
  const __isArchived = (doc.archivedAt != null);
  doc.isArchived = __isArchived;
  ...
  const __timingReturnShopAt = (((doc.timingDepartVenueAt != null) && (doc.timingReturnTravelMinutes != null)) ? (doc.timingDepartVenueAt + (doc.timingReturnTravelMinutes * 60000)) : null);
  ...
  doc.binderColor = __binderColor;
  const __isEditable = ((((doc.stage === "planning") || (doc.stage === "pending_approval")) || (doc.stage === "approved")) || (doc.stage === "sales_lock"));
  doc.isEditable = __isEditable;
  ...
  const __hasServiceStyle = (doc.serviceStyleId != null);
  doc.hasServiceStyle = __hasServiceStyle;
  const __hasFinalLockTiming = ((((doc.timingStaffOnAt != null) && (doc.timingDepartShopAt != null)) && (doc.timingOnsiteAt != null)) && (doc.timingStaffOffAt != null));
  ...
  return {
    ...
    hasServiceStyle: __hasServiceStyle,
    hasFinalLockTiming: __hasFinalLockTiming,
    isFinalLockReady: __isFinalLockReady,
...
/** Computed fields for EventVehicleAssignment. Pass the stored (and hydrated) document. */
export function computeEventVehicleAssignment(doc: Record<string, any>): Record<string, any> {
  const __isPreloaded = (doc.preloadedAt != null);
  doc.isPreloaded = __isPreloaded;
  const __isReleased = (doc.releasedAt != null);
  doc.isReleased = __isReleased;
  return {
    isPreloaded: __isPreloaded,
    isReleased: __isReleased,
  };
}
...
/** Computed fields for PackListItem. Pass the stored (and hydrated) document. */
export function computePackListItem(doc: Record<string, any>): Record<string, any> {
  const __surplusQuantity = Math.max(0, (doc.packedQuantity - doc.requiredQuantity));
  doc.surplusQuantity = __surplusQuantity;
  return {
    surplusQuantity: __surplusQuantity,
  };
}
...
/** Computed fields for Trailer. Pass the stored (and hydrated) document. */
export function computeTrailer(doc: Record<string, any>): Record<string, any> {
  const __isOperational = ((doc.operationalStatus === "available") || (doc.operationalStatus === "in_use"));
  doc.isOperational = __isOperational;
  const __registrationCurrent = ((doc.registrationExpiresAt != null) && (doc.registrationExpiresAt > Date.now()));
  doc.registrationCurrent = __registrationCurrent;
  const __insuranceCurrent = ((doc.insuranceExpiresAt != null) && (doc.insuranceExpiresAt > Date.now()));
  doc.insuranceCurrent = __insuranceCurrent;
  return {
    isOperational: __isOperational,
    registrationCurrent: __registrationCurrent,
    insuranceCurrent: __insuranceCurrent,
  };
}
