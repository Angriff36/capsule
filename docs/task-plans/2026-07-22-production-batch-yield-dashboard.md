...
Add a read-only production dashboard at `/kitchen/yield` that aggregates completed ProductionBatch yield by recipe for a selected rolling window, compares planned and actual output, and ranks the largest under-yield first so kitchen leads can target training.

## Decisions

- Use `ProductionBatch.plannedYield` as expected output and `actualYield` as actual output; recipe catalog yield is a recipe definition, not the expectation for a specific production run.
...
- Rank negative variance from most negative to least negative. Recipes meeting or exceeding plan follow under-yield recipes.
- Keep each recipe and yield unit as a distinct aggregation row so incompatible units are never summed.
...
1. Create `src/features/production/productionYield.ts` with typed batch/recipe inputs and a deterministic report builder for filtering, aggregation, totals, ranking, and date-window metadata.
...
- A completed batch inside the selected window contributes to its recipe totals.
- Draft, in-progress, cancelled, deleted, incomplete, invalid-date, and out-of-window batches do not contribute.
- Two batches for one recipe aggregate by summing planned and actual yield.
...
- The UI names the recipe, shows batch count, expected yield, actual yield, quantity variance, percentage variance, and yield unit.
...
- Temporary Playwright Chromium verification passed after an intentional red run for the missing page. It rendered the real dashboard component with fixture records, confirmed completed-only filtering, 30-day totals, recipe ranking, and the 90-day window recalculation. The temporary spec and harness were deleted; screenshot evidence remains at `output/playwright/production-yield-dashboard.png`.
