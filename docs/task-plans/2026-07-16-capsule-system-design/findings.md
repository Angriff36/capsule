...
- The canonical proofs are behavior-rich, not schema stubs: examples include stock receive/adjust/recount/transfer, inventory reserve/release/consume, recipe draft/publish/retract/retire, menu publish/unpublish, event-dish service composition, prep task claim/start/block/complete, quality pass/fail/reinspect, time clock/correction, qualification grant/revoke/expire, and payroll prepare/finalize/void.
...
- Canonical open decisions remain for timezone/local-time, reapproval, encrypted lists, guest/headcount synchronization, recipe publish completeness, allergen derivation, Menu↔Dish composition, stock mutation side effects, station/training/vehicle concepts, alternate-key reactions, proposal/contract event creation/confirmation, closeout aggregation, allergen automation, and cascade actor capability.
...
- Capsule-Pro provides rich reference coverage for recipes, menus, prep, inventory, procurement, staffing, sales, billing, allergens, and reports; delivery and incidents have little or no obvious route-level coverage, so the canonical model—not Capsule-Pro UI—must lead those experiences.
...
- Dashboard/detail reference: a pale botanical frame and narrow icon rail surround a large white working sheet; overview cards are sparse and editorial, while the recipe detail becomes a dense rule-led form/table without abandoning the same typography or palette.
