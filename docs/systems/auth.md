...
2. Unauthenticated → Capsule username/email and password form. Password submission explicitly uses Clerk password authentication; it does not send a sign-in email code. Set/reset-password is a separate user-selected flow with email ownership verification before choosing a password. Other sign-in options, invitation tickets, and required additional factors remain in the embedded Clerk widget. Ticket links from hire email still sign users in. No self-serve sign-up is added.
