...
The old automatic builder's hard-coded timing windows are removed. Source-backed service/travel/load planning remains required under issue353. The Manifest computed-constraint projection defect is tracked by issue356; this command's effective-window constraint uses arguments/stored values directly. Battle-board print still inherits the narrow Event Day frame (issue357). These branch changes do not establish production or complete binder/report readiness.
