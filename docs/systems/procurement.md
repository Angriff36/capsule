...
Canonical intent is Event approval → demand confirmation → PurchaseNeed creation → VendorOrderLine marks the need ordered. Receipt-to-stock remains an open product/projection decision. Event cancellation stops open purchase work.
...
The UI preserves IngredientDemand and Event provenance and distinguishes explicit operator commands from unverified reactions. It does not claim automatic PurchaseNeed creation, automatic ordered status after adding a line, cancellation fan-out, or receipt-to-stock updates. Money and quantity values retain the current projected-number limitation.
