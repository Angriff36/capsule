# Culinary Component Import and Entity Navigation Design
...
Add a governed component-import workflow for pasted text, `.txt` files, and the existing component CSV exports; make the culinary workspace denser and easier to navigate; and add missing detail routes and links for Ingredient, Dish, and Menu without hand-editing generated output.
...
- the stored Galley component-detail and import references in `docs/design-references/galley`;
- the real catering component and prep materials in `C:/Users/Ryan/Downloads/work`, including mixed-fraction text components, pound notation, CSV component sheets, CSV ingredient lines, and photographed paper components;
...
- Pasted component text.
- `.txt` component files.
- `component_sheet.csv` and `component_lines.csv`, including a paired-file import.
- Durable import provenance and review state.
- Deterministic parsing of component identity, yield, ingredient lines, and method text.
- Exact, possible, and new Ingredient matching with mandatory confirmation for non-exact matches.
- Final creation of a draft Component, confirmed new Ingredients, and ComponentIngredient lines through generated commands.
- A Galley-inspired source/review workspace.
- Compact Component detail layout.
...
- Automatic publishing of imported components.
...
Create `src/culinary/component-import.manifest` and include it from the culinary compile graph.
...
`ComponentImport` is tenant-scoped, soft-deletable, durable, timestamped, and versioned. It records:

- source kind, source filename, raw source text, source byte count, and a deterministic source fingerprint;
- parsed component name, description, category, cuisine, instructions, yield quantity, yield unit, and batch multiplier;
- parsed line count and resolved line count;
- lifecycle status and parsing/finalization failure detail;
- resulting Component ID when known;
- uploaded, parsed, review-started, ready, finalization-started, completed, failed, and cancelled timestamps.

It has many `ComponentImportLine` rows and may reference the resulting Component.

The model must require non-empty source content, positive yield and batch values when present, non-negative line counts, `resolvedLineCount <= parsedLineCount`, and a resulting Component ID before completion.
...
`ComponentImportLine` is tenant-scoped, soft-deletable, durable, timestamped, and versioned. It records:
...
It belongs to `ComponentImport` and may belong to an existing Ingredient.
...
### Permanent Component Provenance

Import-specific source and review state stays on `ComponentImport`. Existing Component fields change only if the final Component requires a permanent, operator-visible value that does not already exist. The resulting Component is linked from the import record rather than duplicating raw import metadata onto every Component.

## Parser Design

The parser is a pure authored TypeScript module under `src/features/kitchen/component-import/`. It has no database access and no generated-file imports beyond consuming generated command input shapes at the integration boundary.
...
The CSV parser supports quoted values, embedded commas, blank optional cells, CRLF/LF input, and paired component-sheet/component-line files. It recognizes files by their headers rather than filenames alone. Rows that cannot be associated with a component remain visible as import errors instead of being dropped.
...
- The right pane contains editable parsed Component fields, yield, method, and Ingredient lines.
...
Finalization remains unavailable until every active line is resolved and required Component fields are valid. Completion links directly to the created Component.
...
Ingredients and method move much closer to the top of the viewport. On suitable desktop widths, composition and method use a balanced working grid. Add/edit forms remain collapsed until requested. Long components may still scroll naturally, but decorative whitespace must not consume the working viewport.
...
- Ingredient detail shows identity, costing/allergen/status facts, and linked Components using it.
- Dish detail shows identity, portion/service facts, linked Component, and linked Event uses when available.
- Menu detail shows identity, pricing/guest/status facts and only relationships present in the generated graph.

### Hyperlinks

Within the kitchen feature, every displayed reference to Component, Ingredient, Dish, or Menu becomes a link when that entity has a detail route. This includes catalog names, Component ingredients, Component dish uses, Ingredient component uses, Dish components, and Event dish selections.

## Loading, Empty, Success, and Error States

- Skeletons match the final header, facts, and workbench geometry.
- Empty source state explains accepted inputs and offers Paste component or Choose files.
...
- Completion links to the created Component and offers another import.
...
- After an unknown Component create, the operator refreshes and links the possibly created Component or explicitly authorizes a new attempt.
...
- Completion is reported only after the resulting Component ID and all expected confirmed ComponentIngredient rows are observable.

## Verification

### Parser Unit Tests

Use sanitized fixtures representing Basil Pesto, Macaroni Salad, Signature Vegetable Medley, component-sheet CSV, and component-line CSV. Cover mixed fractions, catering pound notation, abbreviations, blank quantities, nested methods, quoted CSV cells, malformed rows, and source preservation.

### Manifest and Runtime Tests

- Compile and validate the full Manifest graph.
- Prove allowed and rejected ComponentImport lifecycle transitions.
- Prove per-line resolution constraints and tenant isolation.
- Prove stale-version conflict behavior.
- Prove generated creation is used for Component, Ingredient, and ComponentIngredient.
...
Verify desktop and mobile import layouts, Component detail density, keyboard traversal, visible focus, reduced motion, long ingredient names, long methods, and common small/large result sets against the stored design references.
