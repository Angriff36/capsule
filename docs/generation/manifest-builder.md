...
When disk already equals fresh Builder output but the ledger digest is stale,
regen performs **safe ledger repair** automatically (no file rewrite, no
`adopt ownership`). Use `builder adopt ownership` only for one-time bootstrap
...
Builder records SHA-256 digests in `.builder/ownership.json` and stores the
exact previously generated bytes in `.builder/baselines/<sha256>` (content-
addressed). Regen classifies each owned file three ways:

| Classification       | Condition                           | Behavior                                                 |
| -------------------- | ----------------------------------- | -------------------------------------------------------- |
| `normal-update`      | disk == baseline, candidate differs | Apply candidate; refresh ownership + baseline blob       |
| `safe-ledger-repair` | disk != baseline, disk == candidate | Update ownership digest only (no file rewrite, no adopt) |
| `genuine-conflict`   | disk != baseline, disk != candidate | Stop; print baseline→current and current→candidate diffs |
| `no-op`              | all three equal                     | Do nothing                                               |

Genuine conflicts include digests, both diffs, whether disk matches the fresh
candidate, and whether safe ledger repair is possible. Do **not** hand-edit
`.builder/ownership.json`. Do **not** blind-`adopt ownership` when the
candidate does not match disk.

Common failure modes (any agent, any session):

| Symptom                                                     | Typical cause                                                                                                       |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `owned-file-modified` on `convex/mutations.ts`              | Hand edit or foreign rewrite that differs from both the recorded baseline and the fresh candidate                   |
| `owned-file-modified` on `authContext.ts` / `encryption.ts` | Prettier/`lint-staged` reformatted Builder-owned seams (not in `.prettierignore`); or seam customized without adopt |
...
1. Run `builder generate convex --dry-run` (or `bun run manifest:regen` after a
   conflict-free plan). Read the three-way classification.
2. If the plan lists **safe ledger repairs**, `--apply` updates digests only —
   no adopt step.
3. If the plan shows a **genuine conflict**, inspect the printed diffs. Either
   restore the file to generated output and re-run, or deliberately adopt an
   author seam with `builder adopt ownership` only when you intend to keep
   disk bytes that differ from stock output (`baselined: true`).
4. Never treat “stale digest while disk already matches candidate” as a reason
   to adopt — that is automatic safe ledger repair.
