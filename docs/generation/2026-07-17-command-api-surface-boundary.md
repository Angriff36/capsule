...
No separate AI rules. Prefer capability commands (`Component.draft`,
`ComponentImport.upload` + review/finalize lifecycle) over CRUD on tables.
...
**North star:** In a Cursor/IDE agent chat, you can point at component (or ops)
...
1. **Document → Capsule, agent-driven**  
   Given a local or attached component/ops document (e.g. pasted text, `.txt`,
...
2. **Capability commands only**  
   ...
   | Component                   | `Component.draft` (+ `ComponentIngredient.add` as needed)         |
   | Component import provenance | `ComponentImport.upload` → review/finalize lifecycle           |
   | Dish                     | `Dish.introduce`                                            |
...
5. **Idempotent / safe retry**  
   Retries of the same logical enter operation do not duplicate components/dishes
...
6. **Human-verifiable result**  
   ...
   path) and see the entered ingredients, components, dishes, prep tasks, and/or
   events with correct linkage (e.g. dish→component, event dish→dish).
...
- Outbound partner webhooks (`ComponentDrafted` → POST URL).
- Full Capsule domain coverage beyond the table above (sales, payroll, etc.).

### Demo script (acceptance walkthrough)

1. Place a sample component document in the workspace (or paste into the agent).
2. Prompt: enter this as ingredients + component (+ dish if stated) into Capsule;
...
4. Capsule UI shows the new records; duplicate prompt with same idempotency key
   does not create a second component.

### Status (2026-07-17)

| AC                                       | Status                                                                                                                |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Document → Capsule (component + dish path)  | **Met in proof** — `CapsuleDocumentEnterCoordinator` + `tests/proofs/agent-document-enter.runtime.test.ts`            |
...
| Idempotent retry                         | **Met in proof** — same document re-enter keeps one component/dish                                                       |
| Human-verifiable                         | **Met live** — enter-component returned component/dish ids; visible in Capsule UI                                           |
...
| PrepTask / Event from a component doc alone | **Partial** — use `execute_capsule_command` / mutations with `eventId`/`clientId`; not auto-inferred from component text |
