...
Current delivery status: Slice 1, Event planning foundation, Slice 2, Culinary planning (including `/kitchen/recipes/import` parse→review→createVia finalize DX proof), Slice 3 Demand/stock/purchasing authored surfaces, Slice 4 Kitchen production and quality (prep board + QualityCheck fail→PrepTask block runtime proof), Slice 5 Staffing and time (`/staff` roster, time & availability, and qualifications workspaces + Shift createViaSchedule→start→complete runtime proof), Slice 6 Packing and delivery (`/logistics` pack lists, load sheet, deliveries + PackList→Delivery lifecycle runtime proof), Slice 7 Commercial and billing thin unit (`/finance` invoices + payments + Invoice.issue→send→Payment.record→settle→Invoice.applyPayment runtime proof), and Slice 7b Clients CRM (`/clients` accounts, contacts, proposals, contracts with explicit manual Event follow-ups) are shipped and documented in the owning system pages. Slice 7 still defers PaymentMethod UI and OD035/OD038 automation. Slice 3 keeps blocked automation and projected-number precision explicit; Slice 0 projection-safety backlog items beyond the proven reactions remain open. Slice 5's runtime proof doubles as the regression proof for the manifest 3.6.18–3.6.20 relation-resolution fixes. Slice 6 also regresses opaque FK Zod acceptance for Convex document ids (Manifest 3.6.29+).
