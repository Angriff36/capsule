...
| 2. Culinary planning              | Maintain ingredients/components/dishes, publish menus, and select event dishes                      | Culinary, Events                           | Decide Menu↔Dish composition and component publish completeness                  |
...
Current delivery status: Slices 1–9 plus SavedReportDefinition `/reports` (create→archive→restore library) and Slice 7 PaymentMethod UI (`/finance/payment-methods`, linked from payment record) are shipped. Slice 8 EventCloseout (`/finance/closeout`) and Slice 8b PayrollInput (`/finance/payroll` prepare→finalize runtime proof) remain documented in the owning system pages. Booking conversion and report chart/result rendering now have authored implementations; their full production criteria still require the evidence listed in the current audit. Slice 3 keeps blocked automation and projected-number precision explicit; Slice 0 projection-safety backlog items beyond the proven reactions remain open. Slice 5's runtime proof doubles as the regression proof for the manifest 3.6.18–3.6.20 relation-resolution fixes. Slice 6 also regresses opaque FK Zod acceptance for Convex document ids (Manifest 3.6.29+).

## Production-readiness continuation

The [2026-09-06 criterion audit and delivery sequence](../../codex-plans/production-readiness-next/delivery-sequence.md) applies PR01–14 to released source `cf23268`. The next delivery is [honest, resumable recipe review](../../codex-plans/production-readiness-next/recipe-review-plan.md). Remaining deliveries finish source intake/identity, formulas/stock, financial/booking flows, communications/providers, workforce, logistics and reporting, with access/recovery and qualification alongside them. Earlier slice completion is not full-product certification. These linked plans support this roadmap; they do not start implementation or deployment.
