...
## Execution record (2026-07-28, same day)

Executed in full across commits `956f3bd`, `cb5d6a8` (+ loop wave `645f75e`), `5713bdc`, `885806d`, and the widget-meta follow-up. Status per phase:

- **Phase 0 — DONE.** Status registry (`src/lib/statusLabels.ts`) + StatusChip fallback; all phantom classes replaced (`brand-soft` token + `.btn-secondary` added); ~150 formatter call sites converted (~85 deliberately left: percentages, machine formats, intentional distinct date formats — see commit messages); raw IDs/JSON demoted to copy affordances/inspectors. **Bonus:** ~430 raw Tailwind palette classes (`text-gray-*` etc., invisible to the original audit's phantom-class grep) also swept to theme tokens — they broke dark mode.
- **Phase 1 — DONE.** 1.1 was moot (ledger vocabulary already global). 1.2/1.3: six-area adoption sweep; most pages were already on the masthead/`document-empty` patterns — only genuinely bare headers/loading strings/empty texts were converted. `EmptyState` gained an `action` CTA prop. 1.4: all eight domain failure banners now delegate to the shared classified `FailureBanner`. Two dead CSS classes discovered and fixed en route (`.supply-kv`, `data-table`).
- **Phase 2 — DONE.** Proposals split (1,290→711 lines, form extracted behind disclosure); EventCreate grouped into 4 collapsibles with invalid-field auto-reveal (payload byte-identical); MyDay card-first mobile pass (also removed a duplicated offline banner); Roster forms tucked behind Add buttons; InvoiceDetail document-style masthead + stamp.
- **Phase 3 — DONE** except 3.4 (bottom tab bar — skipped; the Topbar mobile menu already covers the need, recorded above). Grouped sidebar (NAV_GROUPS rendered, separators, drawer = all-workspaces popover); `PlannedAreaPage` + "Future workspaces" deleted; five overview landing pages live (`/finance`, `/staff`, `/logistics`, `/inventory`, `/facilities`) + `/kitchen` lands on the command deck; `FacilitiesWorkspaceNav` added (3.3 parity — remaining areas either have navs or designed tab systems).
- **Phase 4 — DONE** except "paper depth" (dropped as low-value: arrival motion + KPI strips already give landing pages their identity). CSS-only staggered arrival motion (reduced-motion-safe); serif display titles on PageHeader; chip tone story unified via the status registry.

**Verification:** tsc clean tree-wide; 791/792 vitest pass (the 1 failure is the concurrent loop agent's uncommitted manifest regen — createVia count 122 vs 121 — not UI work); nav + finance-route tests green; live browser walkthrough on the local dev deployment confirmed Home, all five overview pages, `/events/new` sections, and My Day render as designed with real data. Proposals page browser-verify was blocked by pre-existing local-backend schema drift (`dishTasks.recipeId`) unrelated to this work — verify it on prod after the next push. Full `bun run check` not run on the shared working tree (concurrent agent's regen state would fail ownership gates through no fault of this work) — run it before pushing.
