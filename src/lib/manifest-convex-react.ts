...
import {
  ...
  RecipeSnapshotCaptureParamsSchema,
...
/** Reactive list for RecipeSnapshot. */
export function useListRecipeSnapshot() {
  return useQuery(api.queries.listRecipeSnapshot);
}

/** Reactive get-by-id for RecipeSnapshot. Pass "skip" to suspend. */
export function useGetRecipeSnapshot(id: string | "skip") {
  return useQuery(api.queries.getRecipeSnapshot, id === "skip" ? "skip" : { id: id as any });
}

/** Mutation hook for RecipeSnapshot.capture. */
export function useRecipeSnapshotCapture() {
  const mutate = useMutation(api.mutations.RecipeSnapshot_capture);
  return (args: any) => {
    const { docId, version, idempotencyKey, ...params } = args ?? {};
    const parsed = RecipeSnapshotCaptureParamsSchema.parse(params) as Record<string, unknown>;
    return mutate({ docId, version, idempotencyKey, ...__convexArgsFromZod(parsed) } as any);
  };
}

/** Governed creation hook for RecipeSnapshot.capture. */
export function useCreateRecipeSnapshot() {
  const mutate = useMutation(api.mutations.RecipeSnapshot_createViaCapture);
  return (args: any) => {
    const { idempotencyKey, ...params } = args ?? {};
    const parsed = RecipeSnapshotCaptureParamsSchema.parse(params) as Record<string, unknown>;
    const body = __convexArgsFromZod(parsed);
