/** The unzipped parts of an .xlsx, by name. The zip step is per platform. */
...
export function readXlsxSheetsFromEntries(entries: ZipEntryMap): XlsxSheet[] {
...
function entryText(entries: ZipEntryMap): (name: string) => string | undefined {
  return (name) => {
    ...
    return entry === undefined
      ? undefined
      : stripNamespacePrefixes(new TextDecoder().decode(entry));
  };
}
 ...
 */
export function readXlsxWorkbookFromEntries(
  entries: ZipEntryMap,
): XlsxTypedWorkbook {
  const text = entryText(entries);
  const workbookXml = text("xl/workbook.xml");
