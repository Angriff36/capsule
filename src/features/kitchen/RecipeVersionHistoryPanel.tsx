...
type Props = {
  ...
  readonly currentData: RecipeSnapshotData;
  readonly canRestore: boolean;
  readonly busy: boolean;
  readonly onRestore: (data: RecipeSnapshotData, versionLabel: string) => void;
};

const CURRENT = "__current__";

export function RecipeVersionHistoryPanel({
  recipeId,
  snapshots,
  currentData,
  canRestore,
  ...
  const history = useMemo(
    () =>
      (snapshots ?? [])
        .filter((row) => row.deletedAt == null && row.recipeId === recipeId)
  ...
  const dataFor = (id: string): RecipeSnapshotData | null => {
    if (id === CURRENT) return currentData;
    const row = history.find((r) => r._id === id);
    return row ? parseRecipeSnapshot(row.snapshot) : null;
  };
  ...
  const scalarDiff = left && right ? diffRecipeScalars(left, right) : [];
  const lineDiff = left && right ? diffRecipeLines(left, right) : [];
    ...
    <section className="culinary-section">
      ...
      {history.length === 0 ? (
        <div className="document-empty">
          ...
          <span>
            Each edit to this recipe captures a snapshot with author and time.
          </span>
        </div>
      ) : (
        <>
              ...
              >
                ...
                {canRestore ? (
                  <button
                    ...
                    disabled={busy}
                    onClick={() => {
                      const data = parseRecipeSnapshot(row.snapshot);
                      if (!data) return;
                      if (
                        !window.confirm(
