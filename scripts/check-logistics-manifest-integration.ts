/**
 * Logistics slice Manifest integration guard — thin wrapper over Manifest proof-kit.
 * Owned tables / lifecycle rules come from generated/proof/guard.logistics.json.
 */
...
export type LogisticsManifestViolation = GuardViolation;

const GUARD_PATH = "generated/proof/guard.logistics.json";
...
export function inspectLogisticsSource(
  relativePath: string,
  source: string,
): LogisticsManifestViolation[] {
...
function inspectOne(
  ...
  if (file.includes("/src/features/logistics/")) {
      ...
      for (const pattern of config.forbiddenImportPatterns) {
        if (new RegExp(pattern).test(imported)) {
          violations.push({
            ...
            detail:
              "Logistics features must use generated React hooks and governed commands.",
          });
        }
      }
    }
    if (
      ...
      violations.push({
        file,
        rule: "approved-api-path",
        detail: "Logistics features must not construct Convex hooks directly.",
      });
    }
    if (
      ...
      violations.push({
        ...
        detail:
          "Logistics lifecycle transitions must come from generated metadata.",
      });
    }
    for (const policy of config.lifecyclePolicies) {
      ...
      if (!source.includes(policy.bindingsImport)) {
        violations.push({
          ...
          rule: "generated-lifecycle",
          detail:
            "LogisticsLifecyclePolicy must consume generated lifecycle metadata.",
        });
      }
  ...
  if (file.includes("/convex/lib/") && config.ownedTables.length) {
    ...
    if (
      ...
      violations.push({
        ...
        detail:
          "Authored Convex modules must write Logistics documents through generated commands.",
      });
    } else if (
  ...
  return violations.map((v) => ({
    ...
    rule:
      v.rule === "approved-api-path"
        ? "approved-logistics-api-path"
        : v.rule === "generated-writes-only"
          ? "generated-logistics-writes-only"
          : v.rule === "generated-lifecycle"
            ? "generated-logistics-lifecycle"
            : v.rule,
  })) as LogisticsManifestViolation[];
}

export function inspectLogisticsManifestIntegration(
  root = process.cwd(),
): LogisticsManifestViolation[] {
...
if (import.meta.main) {
  const violations = inspectLogisticsManifestIntegration();
  if (violations.length) {
    console.error("Logistics Manifest integration guard failed:");
    ...
    console.log(
      "Logistics Manifest integration guard passed: generated APIs and lifecycle metadata remain authoritative.",
    );
  }
}
