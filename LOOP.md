...
| Review gate (L2 — **ON, away mode**)  | Codex (gpt-5.6-sol); cross-model fallback: Fable 5 or grok (Cursor CLI) | Tick pipes worktree diff to `codex exec -s read-only`; REJECT blocks the PR. Reviewer must be a different model than the diff's author; if Codex is unavailable (quota/outage), fall back per loop-constraints.md § Push & Merge                          |
